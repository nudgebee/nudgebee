...
- **Agent** — An LLM-driven worker that handles a domain (e.g., `k8s_orchestrator`, `aws_orchestrator`, `tickets`). Each agent has a system prompt, a planner type, and a tool set. Lives in `llm/llm-server/agents/`.
...
- **Orchestrating agent** — An agent that declares `AgentPlannerTypeOrchestrating` — a top-level "manager" agent (e.g. `k8s_orchestrator`, `aws_orchestrator`) whose job is to coordinate sub-agents across multi-tool investigations. Runs under the ReAct3 planner at runtime. Persisted as `"orchestrating"` in `llm_agents.executor_type` (migration V777; V779 backfilled stragglers and dropped the legacy `"rewoo"` value from the CHECK).
