...
// flows rehydrate when the request omits them. Found reports whether the
// integration was located at all (independent of whether a secret is stored).
...
// LoadExistingConfig returns the stored, decrypted password and auth_type for an
// existing integration located by id or by (tenant, name, tool). Found is false
// only when no integration matches; a located integration with no stored secret
// still returns Found=true so callers can rehydrate other fields. Real database
// errors are propagated so callers can distinguish "not found" from "lookup failed".
//
// On edit flows the frontend may omit the password (leaving the stored secret
// untouched) and never sends auth_type. Both must be rehydrated: credential
// validation branches on auth_type (GitHub App installation ID vs user PAT), so a
// missing auth_type silently mis-validates a GitHub App integration as a token.
func LoadExistingConfig(id, tenant, name, tool string) (ExistingConfig, error) {
	var result ExistingConfig
	if tenant == "" {
		return result, nil
	}
	...
	if id != "" {
		if err := dbManager.Select(&ids, `SELECT id FROM integrations WHERE id = $1 AND tenant_id = $2 LIMIT 1`, id, tenant); err != nil {
			return result, fmt.Errorf("lookup integration by id: %w", err)
		}
	}
	if len(ids) == 0 && name != "" && tool != "" {
		...
		`, tenant, name, tool); err != nil {
			return result, fmt.Errorf("lookup integration by tenant/name: %w", err)
		}
	}
	if len(ids) == 0 {
		return result, nil
	...
	// Fetch both rehydratable config values in one round-trip.
	var rows []struct {
		Name  string `db:"name"`
		Value string `db:"value"`
	}
	if err := dbManager.Select(&rows, `
		SELECT name, value FROM integration_config_values
		WHERE integration_id = $1 AND name IN ('password', 'auth_type')
	`, ids[0]); err != nil {
		return result, fmt.Errorf("lookup stored integration config: %w", err)
	}
	for _, row := range rows {
		...
		case "password":
			...
			plaintext, err := common.Decrypt(row.Value)
			if err != nil {
				return result, fmt.Errorf("decrypt stored password: %w", err)
			}
			result.Password = plaintext
		case "auth_type":
			result.AuthType = row.Value
		}
	}
