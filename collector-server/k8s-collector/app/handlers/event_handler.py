...
def handle_existing_resolved_event(cloud_account_id, existing_event):
    ...
    database.run_query(
        "update events set status = 'CLOSED' , ends_at = now() where cloud_account_id='{}' and id='{}' and"
        " status != 'CLOSED'".format(cloud_account_id, existing_event["id"])
    )
    try:
...
def get_config_change_data(config_change_event, aggregation_key):
    if isinstance(config_change_event[0][2][0]["data"], str):
        config_data = json.loads(config_change_event[0][2][0]["data"])[0]
    else:
        config_data = config_change_event[0][2][0]["data"]
        config_data = {
            "type": "diff",
            "data": config_data,
            "insight": get_change_insights(config_data, aggregation_key),
        }
...
def handle_events(event_data) -> List[Dict[str, Any]]:
    ...
    insights: List[Dict[str, Any]] = []
    if "reason" not in events["headers"] or len(events["headers"]) != 6:
        return insights
    for row in events["rows"]:
        severity = "Critical"
        if row[1] == "Warning" or row[0] == "BackOff":
            insight = {
                "message": row[5],
                "severity": severity,
            }
...
def archive_existing_with_rules(cloud_account_id, tenant, category, rule_names):
    in_clause = "(" + ",".join(["'{}'".format(key) for key in rule_names]) + ")"
    update_to_archieve = f"""update recommendation set status = %s where tenant_id = %s and cloud_account_id = %s and
    category = %s and rule_name in {in_clause}"""
    database.run_query(update_to_archieve, ["Archive", tenant, cloud_account_id, category])
