...
Deployed via Helm to Kubernetes (main/test/prod). Uses AWS ECR registry (`740395098545.dkr.ecr.us-east-1.amazonaws.com`). Image tag format: `{timestamp}_{git-sha}`.

Auto-PR triggered: main merge → test PR, test merge → prod build + deploy.
