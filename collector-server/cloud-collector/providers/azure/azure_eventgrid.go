	...
	for _, subID := range subscriptionIDs {
		...
		for topicsPager.More() {
			...
			for _, topic := range page.Value {
				if topic.ID == nil || topic.Name == nil || topic.Type == nil {
					continue
				}
		...
		for domainsPager.More() {
			...
			for _, domain := range page.Value {
				if domain.ID == nil || domain.Name == nil || domain.Type == nil {
					continue
				}
		...
		for systemTopicsPager.More() {
			...
			for _, systemTopic := range page.Value {
				if systemTopic.ID == nil || systemTopic.Name == nil || systemTopic.Type == nil {
					continue
				}
