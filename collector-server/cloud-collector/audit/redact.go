...
// This file mirrors api-server/services/audit/redact.go. cloud-collector is a
// separate Go module writing to the same shared `audit` table, so the redactor
// is duplicated here rather than imported cross-module — the same rationale under
// which valueOrNil is duplicated across the audit writers. Keep the two in sync.
//
// Here it is applied to event_attr (command args / CLI results), the only
// free-form structured payload cloud-collector persists; event_state and
// event_prev_state are fixed lifecycle strings (STARTED/STOPPED/...).
...
// redactEventAttr returns a redacted copy of an event_attr map, leaving the
// caller's original map untouched. Nil/unredactable input is returned as-is.
func redactEventAttr(attr map[string]any) map[string]any {
	if attr == nil {
		return nil
	}
	if red, ok := redactValue(attr).(map[string]any); ok {
		return red
	}
	return attr
}
