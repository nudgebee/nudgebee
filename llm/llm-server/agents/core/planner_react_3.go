...
func (o *NBReActPlanner3) Plan(
	...
	for range maxOuterIterations {
		...
		if finish != nil {
			...
			critiqueAllowed := o.enableCritique || (config.Config.LlmServerReActCritiqueEnabled && topLevel && IsInvestigationRequestTask(o.request.Query))
			if agent, ok := o.nbAgent.(NBAgentReActPlannerCritiqueSupport); ok {
				critiqueAllowed = critiqueAllowed && agent.CritiqueEnabled()
			}

			if !critiqueAllowed {
				logger.Info("reactagent3: skipping critique", "enableCritique", o.enableCritique, "isTopLevel", topLevel, "isInvestigation", IsInvestigationRequestTask(o.request.Query), "autoCritiqueEnabled", config.Config.LlmServerReActCritiqueEnabled)
				return nil, finish, nil
			}

			decision, feedback := o.runCritique(input, scratchpad, finish.Data, logger)
...
func (o *NBReActPlanner3) runCritique(input, scratchpad, finalAnswer string, logger interface {
	...
	critiquePrompt := prompts.NewPromptTemplate(
		...
		[]string{"input", "scratchpad", "final_answer", "question_type", "tool_names", "tool_descriptions", "shell_tool_enabled"},
	)
