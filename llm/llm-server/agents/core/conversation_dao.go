...
type IConversationDao interface {
	...
	CleanupConversationMessage(id string) error
	...
	GetConversationCost(provider string, model string, nonCachedInputTokens int, cachedInputTokens int, outputTokens int, cacheTTLMinutes *int) (float64, error)
...
func (chat *ConversationDao) CleanupConversationMessage(id string) error {
...
type TokenMetrics struct {
	...
	InputTokens          int             `json:"input_tokens"`            // Total input (cached + non-cached)
	OutputTokens         int             `json:"output_tokens"`           // Total output
	CachedInputTokens    int             `json:"cached_input_tokens"`     // Tokens from cache (cheaper)
	NonCachedInputTokens int             `json:"non_cached_input_tokens"` // New input tokens (regular price)
	...
	CacheTTLMinutes      sql.NullInt64   `json:"cache_ttl_minutes"` // Cache TTL in minutes at time of request
}
...
func CalculateTotalCost(costPerMillionInput, costPerMillionOutput float64, cacheCostPerMillionTokensPerHour sql.NullFloat64, nonCachedInputTokens, cachedInputTokens, outputTokens int, cacheTTLMinutes *int) float64 {
	// Calculate standard costs
	inputCost := (float64(nonCachedInputTokens) / 1000000) * costPerMillionInput
	outputCost := (float64(outputTokens) / 1000000) * costPerMillionOutput

	// Calculate cache storage/maintenance cost (hourly billing)
	cacheStorageCost := 0.0
	if cacheCostPerMillionTokensPerHour.Valid {
		costPerMinute := cacheCostPerMillionTokensPerHour.Float64 / 60.0
		// Use stored TTL if available, otherwise fallback to env variable for backward compatibility
		ttlMinutes := config.Config.LlmCacheTTLMinutes
		if cacheTTLMinutes != nil {
			ttlMinutes = *cacheTTLMinutes
		}
		cacheStorageCost = (float64(cachedInputTokens) / 1000000) * costPerMinute * float64(ttlMinutes)
	}

	return inputCost + outputCost + cacheStorageCost
}
...
func (chat *ConversationDao) GetConversationCosts(models []string) (map[string]modelPricing, error) {
	...
	// Try to get pricing with cache column
	query := `SELECT provider_name, model_name, cost_per_million_input_tokens, cost_per_million_output_tokens, cache_cost_per_million_tokens_per_hour FROM llm_model_pricing`
	...
	if err != nil {
		// Fallback: try without cache column
		query = `SELECT provider_name, model_name, cost_per_million_input_tokens, cost_per_million_output_tokens FROM llm_model_pricing`
		...
		for rows.Next() {
			...
			if err := rows.Scan(&provider, &model, &input, &output); err != nil {
				continue
			}
		...
		for rows.Next() {
			...
			if err := rows.Scan(&provider, &model, &input, &output, &cache); err != nil {
				continue
			}
			pricingMap[provider+":"+model] = modelPricing{
				CostPerMillionInput:              input,
				CostPerMillionOutput:             output,
				CacheCostPerMillionTokensPerHour: cache,
			}
		}
		if err := rows.Err(); err != nil {
			return nil, err
...
func (chat *ConversationDao) GetConversationTokenUsage(conversationId string) ([]TokenMetrics, error) {
	...
	query := `
		SELECT
			...
			t.cache_ttl_minutes as CacheTTLMinutes
		FROM llm_conversation_token_usage t
		INNER JOIN llm_conversations c ON c.id = t.conversation_id
		WHERE c.session_id = $1
		GROUP BY t.agent_name, t.llm_model, t.llm_provider, t.message_id, t.conversation_id, t.cache_ttl_minutes;`
	...
	for rows.Next() {
		...
		if individualAgentStat.ModelProviderName.Valid && individualAgentStat.ModelName.Valid {
			...
			if pricing, ok := pricingMap[key]; ok {
				// Convert sql.NullInt64 to *int for CalculateTotalCost
				var cacheTTLPtr *int
				if individualAgentStat.CacheTTLMinutes.Valid {
					ttl := int(individualAgentStat.CacheTTLMinutes.Int64)
					cacheTTLPtr = &ttl
				}
				individualAgentStat.Cost = CalculateTotalCost(
					pricing.CostPerMillionInput,
					pricing.CostPerMillionOutput,
					pricing.CacheCostPerMillionTokensPerHour,
...
func (chat *ConversationDao) GetConversationCost(provider string, model string, nonCachedInputTokens int, cachedInputTokens int, outputTokens int, cacheTTLMinutes *int) (float64, error) {
	var costPerMillionInputTokens, costPerMillionOutputTokens float64
	var cacheCostPerMillionTokensPerHour sql.NullFloat64

	// First try to get cache pricing if column exists
	rows, err := chat.dbManager.Db.Queryx(`select cost_per_million_input_tokens, cost_per_million_output_tokens, cache_cost_per_million_tokens_per_hour from llm_model_pricing where provider_name = $1 and model_name = $2;`, provider, model)
	if err != nil {
		// Fallback: try without cache column if it doesn't exist
		rows, err = chat.dbManager.Db.Queryx(`select cost_per_million_input_tokens, cost_per_million_output_tokens from llm_model_pricing where provider_name = $1 and model_name = $2;`, provider, model)
		if err != nil {
			return -1, err
		}
	}
	...
	if rows.Next() {
		// Check the columns from the result set to determine which scan to use
		cols, err := rows.Columns()
		if err != nil {
			return -1, err
		}
		if len(cols) == 3 {
			if err := rows.Scan(&costPerMillionInputTokens, &costPerMillionOutputTokens, &cacheCostPerMillionTokensPerHour); err != nil {
				return -1, err
			}
		} else {
			if err := rows.Scan(&costPerMillionInputTokens, &costPerMillionOutputTokens); err != nil {
				return -1, err
			}
			cacheCostPerMillionTokensPerHour = sql.NullFloat64{Float64: 0, Valid: false}
		}
	} else {
		return -1, fmt.Errorf("found no data for model %s and provider %s", model, provider)
	}

	return CalculateTotalCost(costPerMillionInputTokens, costPerMillionOutputTokens, cacheCostPerMillionTokensPerHour, nonCachedInputTokens, cachedInputTokens, outputTokens, cacheTTLMinutes), nil
}
