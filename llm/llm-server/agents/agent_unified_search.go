...
func (a UnifiedSearchAgent) Execute(ctx *security.RequestContext, request core.NBAgentRequest) (core.NBAgentResponse, error) {
	...
	if len(finalResults) == 0 {
		if len(errors) > 0 {
			return core.NBAgentResponse{AgentStepResponse: invocations}, fmt.Errorf("all searches failed: %v", errors)
		}
		return core.NBAgentResponse{
			Response:          []string{"I could not find any relevant information from the configured sources."},
			AgentName:         a.GetName(),
