...
func executeEventAnalysis(ctx *security.RequestContext, c *gin.Context, request any, analysisType events.EventAnalysisType, analysisFunc func(ctx *security.RequestContext, request any) (any, error)) {
	...
	// FF gate — placed AFTER existing-analysis return paths, BEFORE budget
	// check + new IN_PROGRESS writes. Existing RCA stays viewable; new RCA
	// generation is refused when disabled.
	if enabled, _ := common.IsFeatureEnabledByDefaultForAccount(
		"EVENT_AUTO_AI_SUMMARY", ctx.GetSecurityContext().GetTenantId(), accountId,
	); !enabled {
		ctx.GetLogger().Info("analyzer: auto-analysis disabled for this scope, refusing new RCA",
			"event_id", eventId, "account", accountId, "analysis_type", analysisType)
		response.Status = string(events.AnalysisStatusFailed)
		response.StatusReason = "Event analysis is disabled for this account"
		c.JSON(200, buildApiResponse(response, nil))
		return
	}
...
func executeEventInvestigation(ctx *security.RequestContext, request EventAnalysisRequest, c *gin.Context) {
	...
	// FF gate — placed AFTER existing-analysis read & early-returns, BEFORE
	// any new IN_PROGRESS writes. Lets the UI show already-completed
	// analyses while preventing new LLM work for disabled accounts.
	if enabled, _ := common.IsFeatureEnabledByDefaultForAccount(
		"EVENT_AUTO_AI_SUMMARY", ctx.GetSecurityContext().GetTenantId(), request.AccountId,
	); !enabled {
		ctx.GetLogger().Info("analyzer: auto-analysis disabled for this scope, returning existing-only",
			"event_id", request.EventId, "account", request.AccountId)
		finalResponse.Status = string(events.AnalysisStatusFailed)
		finalResponse.StatusReason = "Event analysis is disabled for this account"
		c.JSON(200, buildApiResponse(finalResponse, nil))
		return
	}
...
func analyzeEventRCAUsingAgentsAndUpdateDb(ctx *security.RequestContext, request EventRCAAnalysisRequest) (EventAnalysisResponse, error) {
	// Defense-in-depth — HTTP entry already gates; covers conversation_sync re-trigger path.
	if enabled, _ := common.IsFeatureEnabledByDefaultForAccount(
		"EVENT_AUTO_AI_SUMMARY", ctx.GetSecurityContext().GetTenantId(), request.AccountId,
	); !enabled {
		ctx.GetLogger().Info("analyzer: event RCA disabled for this scope, skipping",
			"event_id", request.EventId, "account", request.AccountId)
		return EventAnalysisResponse{}, nil
	}
...
func analyzeEventUsingAgentsAndUpdateDb(ctx *security.RequestContext, request EventAnalysisRequest) (EventAnalysisResponse, error) {
	// Universal kill switch — covers MQ / HTTP / re-sync / RCA paths.
	if enabled, ffErr := common.IsFeatureEnabledByDefaultForAccount(
		"EVENT_AUTO_AI_SUMMARY",
		ctx.GetSecurityContext().GetTenantId(),
		request.AccountId,
	); ffErr == nil && !enabled {
		ctx.GetLogger().Info("analyzer: auto-analysis disabled for this scope, skipping",
			"event_id", request.EventId,
