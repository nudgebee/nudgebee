...
| `LLM_SERVER_EGRESSFILTER_PII_ENABLED` | `false` | Per-detector knob for the EE PII/PHI scrubber (`ee/scrubbing`). When master is on and this is true, every outbound LLM call routes through `ml-k8s-server /scrub` for reversible tokenization and client-side rehydration on the response. Introduces a runtime dependency on `ml-k8s-server`; flipping on is an explicit ops decision. |
