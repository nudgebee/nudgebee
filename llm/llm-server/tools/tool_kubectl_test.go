...
func TestInferKubectlVerbType(t *testing.T) {
	tests := []struct {
		...
		{"read pipeline with column", "kubectl get pods | column -t", core.ToolRequestTypeRead},
		{"read pipeline with sed", "kubectl get pods | sed 's/foo/bar/'", core.ToolRequestTypeRead},
		{"read pipeline with multiple grep", "kubectl get pods | grep Running | grep -v Completed", core.ToolRequestTypeRead},
		{"read pipeline with grep file", "kubectl get pods | grep -f /etc/passwd", core.ToolRequestTypeRead},
		{"read pipeline with sort file", "kubectl get pods | sort /etc/passwd", core.ToolRequestTypeRead},
		{"pipeline with mutating tail falls through", "kubectl get pods | xargs kubectl delete pod", ""},
