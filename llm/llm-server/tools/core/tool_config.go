...
type AccountConfigSummary struct {
	// IntegrationTypes is ACCOUNT-scoped: integration types linked to this
	// cloud_account via integrations_cloud_accounts.
	IntegrationTypes map[string]bool
	// TenantIntegrationTypes is TENANT-scoped: every enabled integration type in
	// the tenant, regardless of account link. Used by ticket tools, whose config
...
func GetAccountConfigSummary(ctx *security.RequestContext, accountId string) (AccountConfigSummary, error) {
	...
	summary := AccountConfigSummary{
		...
		TenantIntegrationTypes: make(map[string]bool),
	...
	// Fetch tenant-wide integration types (no account link). Ticket integrations
	// are tenant-scoped — they carry no integrations_cloud_accounts row — and the
	// ticket config resolver (ListToolConfigs / ToolConfigSourceTicket(All)) lists
	// them at tenant scope, so the visibility gate reads from here, not the
	// account-scoped IntegrationTypes above.
	g.Go(func() error {
		tenantId := ctx.GetSecurityContext().GetTenantId()
		if tenantId == "" {
			return errors.New("tools: tenantId is required")
		}
		tenantIntegrationRows, err := dbms.Query(`
		SELECT DISTINCT i.type
		FROM integrations i
		...
		for tenantIntegrationRows.Next() {
			var t string
			if err := tenantIntegrationRows.Scan(&t); err == nil {
				mu.Lock()
				summary.TenantIntegrationTypes[strings.ToLower(t)] = true
				mu.Unlock()
			}
		}
		return nil
...
func isToolConfigured(ctx *security.RequestContext, accountId string, tool NBTool, summary AccountConfigSummary, visited map[string]bool) bool {
	...
	case ToolConfigSourceTicket:
		// Single-platform ticket tool (e.g. github). Tenant-scoped to match the
		// resolver (ListToolConfigs / ToolConfigSourceTicket), which lists the
		// integration at tenant scope with no account link.
		return summary.TenantIntegrationTypes[configType]
	case ToolConfigSourceTicketAll:
		...
		ticketTypes := []string{"jira", "github", "gitlab", "servicenow", "pagerduty", "zenduty", "freshdesk", "incidentio"}
