...
// oSeriesReasoningRx matches o1/o3 at the start of the id or just after a
// namespace separator: o1, o3, o1-mini, o1-2024-12-17, openai/o3-mini.
var oSeriesReasoningRx = regexp.MustCompile(`(?i)(^|/)o[13](-|$)`)

// reasoningFamilyRx matches gpt-5 / o4 / o5 delimited by a namespace separator on
// either side, so "openai/gpt-5.6-terra" matches but "gpt-50" does not.
var reasoningFamilyRx = regexp.MustCompile(`(?i)(^|[/:._-])(gpt-5|o[45])($|[/:._-])`)
...
var modelCapabilities = []ModelCapability{
	...
	{
		// NUDGEBEE: was `(?i)^o[13](-mini|-preview)?$`, an exhaustive-suffix list
		// anchored to the whole string. It missed every dated checkpoint
		// (o1-2024-12-17) and every namespaced id a gateway serves (openai/o1),
		// so those models were treated as system-message-capable and sent one.
		Pattern:          `(?i)(^|/)o[13](-|$)`, // o1, o3, o1-mini, o1-2024-12-17, openai/o3-mini
...
func (o *LLM) GenerateContent(ctx context.Context, messages []llms.MessageContent, options ...llms.CallOption) (*llms.ContentResponse, error) { //nolint: lll, cyclop, funlen
	...
	for _, mc := range messages {
		...
		msg.MultiContent = newParts
		convertedCalls, err := toolCallsFromToolCalls(toolCalls)
		if err != nil {
			return nil, err
		}
		msg.ToolCalls = convertedCalls

		chatMsgs = append(chatMsgs, msg)
	}
	// NUDGEBEE: the salvage above only runs on a human message. Without one, a model
	// that takes no system role lost its system prompt entirely and silently. Widening
	// the o-series match made that reachable for namespaced ids like "openai/o1".
	if systemContent != "" && len(chatMsgs) > 0 {
		chatMsgs[0].MultiContent = append(
			[]llms.ContentPart{llms.TextContent{Text: systemContent + "\n\n"}},
			chatMsgs[0].MultiContent...,
		)
	}
...
func (o *LLM) SupportsReasoning() bool {
	...
	// NUDGEBEE: the prefix/substring checks here missed the bare "o1" and any
	// namespaced id ("openai/o1"), so a reasoning model reached through a
	// gateway was not recognised as one. Same matcher as the capability table.
	if oSeriesReasoningRx.MatchString(modelLower) {
		return true
	}

	// GPT-5 and future o4/o5 series.
	// NUDGEBEE: these were prefix checks, so "openai/gpt-5.6-terra" — the id format
	// our own gateway serves — did not match. Boundary-aware to mirror
	// isOpenAIReasoningModel in agents/core, which already matches this way.
	if reasoningFamilyRx.MatchString(modelLower) {
		return true
	}
...
func toolCallsFromToolCalls(tcs []llms.ToolCall) ([]openaiclient.ToolCall, error) {
	toolCalls := make([]openaiclient.ToolCall, len(tcs))
	for i, tc := range tcs {
		// NUDGEBEE: ExtractToolParts passes any llms.ToolCall through unchecked, so a
		// malformed one reaches here and dereferences nil. Fail the request instead of
		// panicking, and instead of emitting a nameless call the API cannot dispatch.
		if tc.FunctionCall == nil {
			return nil, fmt.Errorf("tool call %q has no function call", tc.ID)
		}
...
func toolCallFromToolCall(tc llms.ToolCall) openaiclient.ToolCall {
	// NUDGEBEE: a call with no arguments streams back as "", which is not valid
	// JSON. Replaying that into history makes the next request 400.
