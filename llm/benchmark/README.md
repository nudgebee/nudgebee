...
| `LLM_SERVER_URL` | Yes | LLM server base URL |
| `BENCHMARK_PORT` | No | Server port (default: 9999) |
| `DASHBOARD_PORT` | No | Streamlit port (default: 8501) |
| `BENCHMARK_API_URL` | No | URL dashboard uses to reach server (default: http://localhost:9999) |
| `UVICORN_WORKERS` | No | Server worker count (default: 2) |
| `ACCOUNT_ID` | Test | Account for LLM calls |
| `USER_ID` | Test | User for LLM calls |
| `TENANT_ID` | Test | Tenant for LLM calls |
