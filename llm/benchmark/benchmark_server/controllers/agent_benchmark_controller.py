...
async def run_agent_benchmark_and_notify(  # noqa: C901
    ...
    if infra_state and infra_state.get("status") == "deployed":
        ...
        try:
            ...
            agent_dir = (
                Path(__file__).resolve().parent.parent.parent / "llm" / "agents" / agent
            )
            needed = set(
...
async def trigger_agent_benchmark(
    request: AgentBenchmarkRequest, background_tasks: BackgroundTasks
):
    """
...
async def gather_benchmark_tests(request: GatherBenchmarkRequest):
...
async def deploy_standalone_infra(
    req: InfraDeployRequest, background_tasks: BackgroundTasks
):
...
async def nuke_standalone_infra(
    req: InfraNukeRequest, background_tasks: BackgroundTasks
):
...
async def cancel_infra_operation(req: InfraNukeRequest):
    """Cancel an in-progress deploy or nuke operation."""
    agent = req.agent
    current = _get_infra_state(agent)
...
async def reset_infra_state(req: InfraNukeRequest):
    """Force-reset a stuck infra state.
...
async def get_infra_status(agent: Optional[str] = None):
...
async def list_benchmark_runs(
    ...
    if _last_orphan_sweep_at is None or (now - _last_orphan_sweep_at) >= _ORPHAN_SWEEP_INTERVAL:
...
async def cleanup_benchmark_runs():
    """Remove state files for all completed and stopped benchmark runs."""
    removed = run_manager.cleanup_completed_runs()
    return {
        "removed": removed,
...
async def run_single_test(
    run_id: str, test_index: int, background_tasks: BackgroundTasks
):
...
async def run_all_gathered_tests(run_id: str, background_tasks: BackgroundTasks):
...
async def rerun_benchmark(run_id: str, background_tasks: BackgroundTasks):
...
async def rerun_specific_tests(
    run_id: str, request: RerunTestsRequest, background_tasks: BackgroundTasks
):
...
async def restart_benchmark(run_id: str, background_tasks: BackgroundTasks):
...
async def re_evaluate_benchmark(run_id: str, background_tasks: BackgroundTasks):
...
async def _run_re_evaluation(run_id: str):
    ...
    try:
        batch_scores = evaluate_batch(queries, answers, references, llm, eval_times=eval_times)
...
async def download_benchmark_report(run_id: str):
...
async def resend_report(run_id: str, request: ResendReportRequest):
...
async def compare_benchmark_runs(request: CompareRunsRequest):
