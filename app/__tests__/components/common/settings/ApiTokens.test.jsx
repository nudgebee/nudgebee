...
jest.mock('@ui/Modal', () => ({
  __esModule: true,
  Modal: ({ open, onClose: _onClose, title, children, actionButtons }) =>
    open ? (
...
jest.mock('@ui/Button', () => ({
  ...
  Button: ({ children, onClick, disabled, loading }) => (
    <button onClick={onClick} disabled={disabled || loading} data-testid={`btn-${children}`}>
      {children}
    </button>
  ),
}));
    ...
    <table>
      <tbody>
        {tableData.map((row, i) => (
          <tr key={i}>
            {row.map((cell, j) => (
              <td key={j}>{cell.component || cell.text}</td>
            ))}
          </tr>
        ))}
      </tbody>
...
jest.mock('@shared/icons/SafeIcon', () => ({
  __esModule: true,
  default: ({ alt, ...props }) => React.createElement('img', { alt, 'data-testid': 'safe-icon', ...props }),
}));

jest.mock('@assets', () => ({
  DeleteIconRed: 'delete-icon-red.svg',
}));
...
describe('ApiTokens', () => {
  ...
  it('calls createUserToken and shows success on token creation', async () => {
    ...
    await waitFor(() => {
      ...
      expect(toast.success).toHaveBeenCalledWith('API Token created successfully');
    });
  });
