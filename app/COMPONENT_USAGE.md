...
in each file's JSDoc header (e.g. [`ds/ListingLayout.tsx`](src/components/common/ds/ListingLayout.tsx)
...
`ds/` = design-system primitive (`@ui/*`). `common/` = domain composition
(`@shared/*`) — an app-specific component built from primitives.
...
| `CodeBlock`                                           | ds/   | **Display** a static code snippet, shell command, or config fragment with a copy button. Props: `code` (required), `language`/`title` (header), `inline` (chip in running text), `tone` (`light`/`dark` terminal), `showLineNumbers`, `prompt` (e.g. `$`), `wrap`, `maxHeight`.                                                                                                                                          |
| `CodeEditor`                                          | ds/   | **Edit** code, or display **read-only** code with syntax highlighting / line numbers / folding. CodeMirror wrapper; shares `CodeBlock`'s surface. Props: `value` (required), `onChange`, `language` (`yaml`/`json`/`sql`/`javascript`/`shell`/`markdown`/`text`), `readOnly`, `tone`, `height`, `title`, `extensions` (escape hatch for PromQL/lint). Use instead of a raw `<CodeMirror>`.                               |
| `DiffViewer`                                          | ds/   | Show **what changed** between two versions. Engine inferred from input: `gitDiff` (unified string) → unified +/- rows; `originalCode`+`newCode` → split side-by-side (CodeMirror MergeView). `mode='unified'\|'split'` overrides. Props: `language`, `title`/`fileName`, `showHeader`, `collapsible`, `leftLabel`/`rightLabel`, `maxHeight`. Replaces `@shared/viewers/SimpleDiffViewer` + `@shared/viewers/DiffViewer`. |
...
| `FilterDropdown`                                 | ds/     | Value picker for a **toolbar / filter bar** — inline pill trigger with clear-X. See §3 for "form vs filter" rule. An option's `icon` (svg-asset src or JSX) renders as a 16px leading `SafeIcon` in its row. Panel width defaults to the trigger width (220px floor); `popoverWidth` overrides.                                                                                  |
...
- `common/CustomTextField` (legacy `components/common/`) — still in use; migrate to `Input` opportunistically.
...
| Component                        | Where   | Use when                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| -------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CustomTabs`                     | common/ | **Canonical tabs primitive for this project.** Import: `import CustomTabs from '@shared/CustomTabs'`. Single-level tabs with sliding pill indicator. `behavior='filter'` for state-only (modals, in-page toggles); `behavior='router'` for URL-driven nav. `variant='primary'` (34px tab, brand-600 underline 2px below) for top-level nav; `secondary` (30px, no underline) for inner subtabs. Set `showSurface={false}` for naked tab strips. **Per-tab icon size:** pass `iconSize: <pixels>` on individual `tabOptions` entries to override the per-variant default (20px primary, 18px secondary). **Use this — not `ds/Tabs`.** |
| `Tabs`                           | ds/     | DS V2 tabs primitive. **Not the default in this project** — use `@shared/CustomTabs` instead. Kept available for future re-evaluation when Phase 2 visual styling lands.                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `Banner` · `Toast`                                                             | ds/     | Inline page banners / transient toast notifications. **Toast is imperative** — `import { toast } from '@ui/Toast'`, mount `<Toast />` once in `_app.tsx`, then `toast.success('…')`. Severities, `description`, `action`. See the **Toast** note below.                                                                                                                                                   |
...
- **Severities:** `toast.default` (plain **white** card, **no** icon — for neutral text) · `toast.success` · `toast.info` · `toast.warning` · `toast.error`. Severity drives the card colour + filled icon. Auto-dismiss runs on a per-severity timer (≈3–6 s); **hover pauses** it.
- **Options bag** — 2nd arg `{ … }`, or a bare number as a `duration` shorthand:
  - `description` — a smaller, gray sub-line under the message.
  - `action` — `{ label, onClick }` renders a `Button` (`tone='secondary'`) before the close; clicking runs `onClick` **and then dismisses** the toast.
  - `duration` — override the per-severity default (ms).
- Stacks **top-right**, newest on top, **max 3 visible**, **fixed width** (long messages wrap). Toasts animate in (slide-down + fade + small scale "pop") and out (slide-up + fade); neighbours reflow smoothly. All motion respects `prefers-reduced-motion`.
...
Full anatomy, motion spec and Don'ts: [`ds/Toast.tsx`](src/components/common/ds/Toast.tsx) /
[`SnackbarComponent.tsx`](src/components/common/SnackbarComponent.tsx) JSDoc and the design-system
Toast page (`design-system/primitives/feedback/toast.html`). **Don't** put the only copy of critical
info in a Toast — it vanishes whether or not it's read (use `Banner` / the notification centre).
...
**Anatomy** (from [`ds/Modal.tsx`](src/components/common/ds/Modal.tsx) — read its JSDoc):
...
- Don't introduce new uses of legacy `@shared/modal` or
  `@shared/modal/NDialog` — those stay live during migration but every new dialog goes
  through `@ui/Modal`.
...
| In-page tab switch (state only) — modals, panels, in-component toggles                             | `@shared/CustomTabs` with `behavior='filter'`                                         | `ds/Tabs` (not the default in this project)                                                           |
| Tabs that drive the URL / route                                                                    | `@shared/CustomTabs` with `behavior='router'`                                         | `ds/PageTabs` (page-level only) · `ds/Tabs` (not default here)                                        |
| Top-of-page **2-level** nav — parent tabs with hover-dropdown subtabs, URL-hash driven             | `common/AnchorComponent` (wraps `@shared/CustomTabs` for the subtab row)              | `ds/PageTabs` (single-level only) · `ds/Tabs` (state-only, no dropdown)                               |
...
| One collapsible unit                                                                               | `CollapsableCard`                                                                     | `Accordion` ("Don't use Accordion for < 3 rows")                                                      |
| 3+ sibling collapsibles                                                                            | `Accordion`                                                                           | stacking `CollapsableCard`s                                                                           |
| In-page tab switch (state only)                                                                    | `@shared/CustomTabs` with `behavior='filter'`                                         | `ds/Tabs` (not the default in this project)                                                           |
| Tabs that drive the URL / route                                                                    | `@shared/CustomTabs` with `behavior='router'`                                         | `ds/PageTabs` (page-level only)                                                                       |
| Transient confirmation message                                                                     | `Toast`                                                                               | `Banner`                                                                                              |
| Persistent inline page-level message                                                               | `Banner`                                                                              | `Toast`                                                                                               |
| Small overlay anchored to an element                                                               | `Popover` (rich) · `Tooltip` (text)                                                   | `Modal`                                                                                               |
| Centered blocking task                                                                             | `Modal` / `Dialog`                                                                    | `Inspector`                                                                                           |
...
| Show a static code snippet / shell command with a copy button (read-only)                          | `ds/CodeBlock`                                                                        | A hand-rolled `<pre>` + `CopyButton` (loses tokens, header, line numbers, prompt, dark tone)          |
| Inline `code` chip inside a sentence                                                               | `<CodeBlock inline code='…' />`                                                       | A raw `<code>` tag (no mono token, no surface/border tokens)                                          |
| Prose / markdown that _contains_ fenced code blocks                                                | `ds/Markdown` (`@shared/viewers/MarkDowns`)                                           | `ds/CodeBlock` (CodeBlock shows one snippet; it doesn't parse markdown)                               |
| Editable / language-aware code (YAML/JSON/SQL/JS/Shell/Markdown)                                   | `ds/CodeEditor`                                                                       | A raw `<CodeMirror>` (re-wires language/theme/readOnly per call site) · `ds/CodeBlock` (display-only) |
| Read-only code that needs **syntax highlighting / line numbers / folding**                         | `ds/CodeEditor` with `readOnly`                                                       | `ds/CodeBlock` (no highlighting — structural mono only)                                               |
| Read-only code, **no** highlighting needed — just show + copy                                      | `ds/CodeBlock`                                                                        | `ds/CodeEditor` (heavier — pulls in CodeMirror for a static snippet)                                  |
| PromQL editor, or any editor needing lint / custom autocomplete                                    | `ds/CodeEditor` + the PromQL/lint extensions via its `extensions` prop                | A raw `<CodeMirror>` (loses the DS surface chrome)                                                    |
