...
const KubernetesWorkloadsTable = ({ accountId, resource_ids = [] }) => {
  ...
  const fetchGitDetails = async (workload) => {
    ...
    const ciRepoAnnotation = annotations['ci.nudgebee.com/git.repo'];
    const ciHash = annotations['ci.nudgebee.com/git.hash'];
    const ciBranch = annotations['ci.nudgebee.com/git.branch'] || 'main';
    const ciValuesPath = annotations['ci.nudgebee.com/helm.values.filePath'] || '';
    const workloadRepoAnnotation = annotations['workloads.nudgebee.com/git.repo'];
    const workloadHash = annotations['workloads.nudgebee.com/git.hash'];
    ...
    try {
      ...
      const ciRepoAttr = attributes.find((a) => a.name === 'ci.nudgebee.com/git.repo');
      const branchAttr = attributes.find((a) => a.name === 'ci.nudgebee.com/git.branch');
      const valuesAttr = attributes.find((a) => a.name === 'ci.nudgebee.com/helm.values.filePath');
      const workloadRepoAttr = attributes.find((a) => a.name === 'workloads.nudgebee.com/git.repo');
  ...
  const handleSaveGitDetails = async () => {
    ...
    try {
      ...
      await k8sApi.upsertResourceAttributes(gitModalWorkload.cloud_resource_id, accountId, [
        ...
        { name: 'workloads.nudgebee.com/git.repo', value: gitDetails.codeRepo },
        // CI annotations for PR creation
        { name: 'ci.nudgebee.com/git.repo', value: ciRepoValue },
        { name: 'ci.nudgebee.com/git.branch', value: gitDetails.branch },
        { name: 'ci.nudgebee.com/helm.values.filePath', value: gitDetails.valuesFilePath },
      ]);
