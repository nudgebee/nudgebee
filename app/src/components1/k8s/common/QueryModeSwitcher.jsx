    ...
    try {
      ...
      blocks.forEach((block) => {
        (block.queryItems || []).forEach((item) => {
          if (normalizeLegacyOperator(item.operator) === '_eq' && item.label && item.value) {
      ...
      const results = await Promise.all(
        blocksWithMetrics.map(async (block) => {
          const response = await observability.getMetricsQuery({
            account_id: accountId,
            queries: { [block.query_key]: block.selectedMetric },
            labels,
          ...
          return response?.data?.data?.metrics_get_query?.query || null;
        })
      );

      const formattedQuery = results.filter(Boolean).join(';');
      if (formattedQuery) {
