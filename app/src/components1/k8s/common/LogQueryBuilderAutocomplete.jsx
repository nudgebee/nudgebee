...
import { getLineOperators, dynatraceMetricAggregators } from './QueryBuilder';
    ...
    <Box sx={{ width: '100%', minHeight: height, display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      {queryBlocks.map((block, index) => (
        <Box key={block.id} sx={{ display: 'flex', flexDirection: 'row', gap: 2, width: '100%' }}>
          ...
          {/* Operations for Loki, New Relic, Dynatrace, and Loggly */}
          {['loki', 'newrelic', 'dynatrace', 'loggly'].includes(logProvider) && activeBlockId === block.id && (
            <Box sx={{ width: '50%', padding: '12px 16px', border: '1px solid #E5E7EB', borderRadius: '6px', mt: '5px' }}>
              ...
              {block.operations.map((op) => (
                <Box key={op.id} sx={{ display: 'flex', alignItems: 'flex-start', gap: 1, height: '44px' }}>
                  <FilterDropdownButton
                    options={
                      logProvider === 'dynatrace' && providerType === 'metrics' ? dynatraceMetricAggregators : getLineOperators(operatorDescriptors)
                    }
                    value={
                      (logProvider === 'dynatrace' && providerType === 'metrics'
                        ? dynatraceMetricAggregators
                        : getLineOperators(operatorDescriptors)
                      ).find((o) => (o.value ?? o) === op.op) || null
                    }
                    label='Operation'
                    onSelect={(event) => {
