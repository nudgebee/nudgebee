...
import { ArrowRightWhiteIcon, ShareIconBlue, SuccessIcon, SaveIconOutlineselect } from '@assets';
...
import { Textarea } from '@components1/k8s/common/TextAreaV2';
...
const KubernetesLLMRequestResponse = (props) => {
  ...
  const [followupResponse, setFollowupResponse] = useState(props.toolCall?.response?.text);
  ...
  // Disable followup when it was terminated or conversation moved past it
  const followupStatus = props.toolCall?.response?.status;
  const isFollowupExpired =
    props.toolCall?.type === 'followup-question' && (followupStatus === 'TERMINATED' || followupStatus === 'KILLED' || followupStatus === 'FAILED');

  useEffect(() => {
    // Clear selectedOption when response text changes (indicating new response received)

    if (props.toolCall?.response?.text && selectedOption) {
      setSelectedOption(null);
    }
  }, [props.toolCall?.response?.text, selectedOption]);
  ...
  const getToolResponseCard = function () {
    ...
    if (toolName == 'TroubleshootPlanner' && props.toolCall?.log) {
      ...
      return (
        <>
          {messageConfig.followupType == 'multi_select' ? (
            <>
              {messageConfig.question && (
                <Box sx={{ mb: 2, p: 2, backgroundColor: '#f5f5f5', borderRadius: 1 }}>
                  <MarkDowns data={messageConfig.question} />
                </Box>
              )}
              <Text value='Options (select one or more)' sx={{ fontWeight: '500', mb: '8px' }} />
              <Box sx={{ display: 'flex', gap: '16px', flexWrap: 'wrap', mb: '16px' }}>
                {messageConfig?.followupOptions?.map((option) => {
                  const isCompleted = props.toolCall?.response?.status == 'COMPLETED';
                  const isSelected = isCompleted ? props.toolCall?.response?.text?.includes(option) : selectedMultiOptions.includes(option);
                  return (
                    <Box key={option} sx={isSelected ? { border: '1.5px solid ' + colors.primary, borderRadius: '4px', padding: '0px' } : {}}>
                      <CustomIconButton
                        size='small'
                        variant='outline'
                        sx={{
                        ...
                        onClick={() => {
                          setSelectedMultiOptions((prev) => (prev.includes(option) ? prev.filter((o) => o !== option) : [...prev, option]));
                        }}
                        key={option}
                        isDisabled={isSubmitting || isCompleted || isFollowupExpired}
                      >
                        {option}
                        {isSelected ? (
                          <Box sx={{ ml: '8px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                            <SafeIcon src={SuccessIcon} width={16} height={16} />
                          </Box>
                        ) : null}
                      </CustomIconButton>
                    </Box>
              ...
              {props.toolCall?.response?.status != 'COMPLETED' && !isFollowupExpired && (
                <CustomIconButton
                  size='small'
                  variant='blueButton'
                  sx={{
                  ...
                  onClick={() => {
                    handleFollowupResponse(null, selectedMultiOptions);
                  }}
                  isDisabled={isSubmitting || selectedMultiOptions.length === 0}
                >
                  Submit
                </CustomIconButton>
              )}
          ...
          ) : messageConfig.followupType == 'single_select' ||
            messageConfig.followupType == 'tool_config' ||
            messageConfig.followupType == 'tool_confirmation' ? (
            <>
              {messageConfig.question && (
                <Box sx={{ mb: 2, p: 2, backgroundColor: '#f5f5f5', borderRadius: 1 }}>
                  <MarkDowns data={messageConfig.question} />
              ...
              <Text value='Options' sx={{ fontWeight: '500', mb: '8px' }} />
              <Box sx={{ display: 'flex', gap: '16px', flexWrap: 'wrap', mb: '16px' }}>
                {messageConfig?.followupOptions?.map((option) => (
                  <Box
                    key={option}
                    sx={
                      followupResponse.includes(option) ||
                      props.toolCall?.response?.text.includes(option) ||
                      props.toolCall?.response?.text.includes(option)
                        ? { border: '1.5px solid ' + colors.primary, borderRadius: '4px', padding: '0px' }
                        : {}
                    }
                  >
                    <CustomIconButton
                      ...
                      key={option}
                      isDisabled={isSubmitting || props.toolCall?.response?.status == 'COMPLETED' || isFollowupExpired}
                    >
                      {option}
                      {followupResponse.includes(option) || props.toolCall?.response?.text.includes(option) ? (
                        <Box sx={{ ml: '8px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                          <SafeIcon src={SuccessIcon} width={16} height={16} />
                        </Box>
                      ) : null}
            ...
            <>
              {isFollowupExpired ? (
                <Typography sx={{ fontSize: '13px', color: colors.text.tertiary, fontStyle: 'italic', mt: 1 }}>
                  Conversation was terminated. This question was not answered.
                </Typography>
              ) : (
                (() => {
                  const isFollowupDisabled = isSubmitting || props.toolCall?.response?.status == 'COMPLETED';
                  // Use the same effective value the textarea displays so the counter and the
                  // empty-check stay in sync with what the user actually sees.
                  const effectiveFollowupResponse = followupResponse ?? props.toolCall?.response?.text ?? '';
                  const responseLength = effectiveFollowupResponse.length;
                  const hasContent = effectiveFollowupResponse.trim().length > 0;
                  const submitFollowup = () => {
                    if (!isFollowupDisabled && hasContent) {
                    ...
                    >
                      ...
                      <Box
                        sx={{
                          ...
                          alignItems: 'center',
                          gap: '8px',
                          border: `1.5px solid ${colors.border.primaryLight}`,
                          borderRadius: '10px',
                      ...
                      >
                        <Textarea
                          ...
                          width='100%'
                          minRows={1}
                          value={followupResponse ?? props.toolCall?.response?.text}
                        ...
                        />
                        <CustomButton
                          id='followup-response-submit-btn'
                          label='Send follow-up response'
                          size='Medium'
                          variant='blueButton'
                          sx={{
                            flexShrink: 0,
                            ...
                            backgroundColor: isFollowupDisabled || !hasContent ? colors.background.primaryLight : colors.primary,
                            '&:hover': {
                              backgroundColor: isFollowupDisabled || !hasContent ? colors.background.primaryLight : colors.button.primaryHover,
                            },
                          }}
                          onClick={submitFollowup}
                          disabled={isFollowupDisabled || !hasContent}
                      ...
                      >
                        ...
                        <Box sx={{ fontVariantNumeric: 'tabular-nums', flexShrink: 0 }}>{responseLength} / 550</Box>
                      </Box>
                    </Box>
                  );
                })()
  ...
  const handleFollowupResponse = async (option, multiOptions) => {
    ...
    try {
      const agentId = props.toolCall?.response?.agent_id;
      const parentAgentId = props.toolCall?.response?.parent_agent_id;

      const resolvedParentAgentId = parentAgentId === ZERO_UUID || !parentAgentId ? agentId : parentAgentId;

      const requestPayload = {
        account_id: props.toolCall?.response?.account_id,
        query: multiOptions?.length > 0 ? JSON.stringify(multiOptions) : option || followupResponse,
        conversation_id: props.conversationId,
        message_id: props.toolCall?.response?.message_id,
        agent_id: agentId,
        parent_agent_id: resolvedParentAgentId,
      };

      // Add model configuration if a model is selected
      if (props.selectedModel) {
        requestPayload.config = {
          llm_provider: props.selectedModel.provider,
          llm_model_name: props.selectedModel.model,
        };
      }
            ...
            <>
              ...
              <Grid item md={10} mt='12px'>
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-start', width: '100%', gap: '8px' }}>
                  <Text
                    value={props.toolCall.type == 'followup-question' ? ' ' : 'Agent Response'}
                    sx={{ fontWeight: '400', mb: '4px', fontFamily: 'Roboto', fontSize: '13px' }}
                  />
                  {props.toolCall.type !== 'followup-question' && (
                    <CopyableText
                      copyableText={
