...
const KubernetesLLMResponseGenerator = ({
  ...
  // Load conversation history in popup mode when sessionId is provided via props
  // (e.g., from workflow AI generation where the conversation already exists in the DB)
  // Skip fetch when queryPrefix is provided — the session will be created on first query submission
  useEffect(() => {
    if (popup && sessionId && !currentlyProcessingQuestion) {
      if (sessionId !== selectedSessionId) {
        setSelectedSessionId(sessionId);
      }
      fetchConversation(sessionId, '', 'selected', false);
    }
  }, [popup, sessionId, queryPrefix]);
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                ...
                {selectedSessionId == '' && selectedConversationId == '' && !popup && (
                  <Box
          ...
          {(selectedSessionId != '' || selectedConversationId != '') && (conversationStatus !== 'NOT_FOUND' || queryPrefix) ? (
            <Box
              sx={{
                display: 'flex',
                flexDirection: 'column',
              ...
              {!showFollowupSheet && (
                ...
                >
                  <AutoSuggestTextarea
                    ...
                    isFollowUp={true}
                    allowStop={allowStop && messages.length > 0}
                  />
                </SummaryBlock>
              )}
              {!popup && (
