    ...
    if (typeof parsed === 'string') {
      ...
      if (inner.startsWith('{') || inner.startsWith('[')) {
        try {
          ...
          // leave parsed as the inner string
        }
      }
    }
    if (parsed && typeof parsed === 'object') {
      ...
      if (!Array.isArray(parsed)) {
        return (
          <Box sx={{ backgroundColor: colors.background.codeOutput, borderRadius: '8px', p: '12px', overflowX: 'auto' }}>
  ...
  if (isKubectlTool(toolName)) {
    try {
      ...
      if (results.stdout || results.stderr) {
        return (
          <Box sx={{ backgroundColor: '#2d3748', borderRadius: '8px', p: '12px', overflowX: 'auto' }}>
            <pre style={preStyle}>{(results.stdout || results.stderr).replace(/\\n/g, '\n')}</pre>
          </Box>
        );
      }
    } catch {
