...
const KubernetesSecurityDetails = (props) => {
  ...
  const filteredGitIntegrations = useMemo(() => {
    const repoUrl = selectedWorkloadAnnotations['ci.nudgebee.com/git.repo'] || selectedWorkloadAnnotations['workloads.nudgebee.com/git.repo'];
    ...
    try {
      ...
      if (workloads.length === 1) {
        ...
        const filteredKeys = Object.keys(annotations).filter(
          (key) => key.startsWith('workloads.nudgebee.com') || key.startsWith('ci.nudgebee.com') || key.startsWith('argocd.argoproj.io')
        );
        ...
        if (workloads[0].cloud_resource_id) {
          ...
          attributes.forEach((attr) => {
            ...
            if (attr.name.startsWith('workloads.nudgebee.com') || attr.name.startsWith('ci.nudgebee.com')) manualConfig[attr.name] = attr.value;
          });
          ...
          Object.keys(selectedWorkloadAnnotations).length > 0 ? (
            ...
            <Box sx={{ p: 2 }}>
              <Typography color='warning.main'>
                No CI annotations found for this workload. Please configure CI annotations (ci.nudgebee.com/git.repo,
                ci.nudgebee.com/helm.values.filePath) on the workload.
              </Typography>
