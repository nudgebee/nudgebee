...
const ApiTokens = ({ open, title, onClose }) => {
  ...
  const getCurrentBaseUrl = () => {
    ...
    return process.env.BASE_URL || 'https://app.nudgebee.com';
  };
    ...
    <>
      ...
      >
        <Box sx={{ color: colors.text.tertiary, fontSize: '14px', lineHeight: '20px' }}>
          ...
          >
            {`curl ${getCurrentBaseUrl()}/api/auth/token --data '{"email":"your@email.com", "secret":"YOUR_API_TOKEN"}' -i -H 'content-type: application/json'`}
          </Box>
          ...
          >
            {`curl ${getCurrentBaseUrl()}/api/graphql -i -H 'content-type: application/json' -H "Authorization: Bearer $AUTH_TOKEN" --data $QUERY_DATA`}
          </Box>
          ...
          <Box component='ul' sx={{ pl: '20px', mb: '12px' }}>
            ...
            <li style={{ marginBottom: '8px' }}>
              <strong>Current Environment:</strong> Commands above use your current domain ({getCurrentBaseUrl()})
            </li>
            <li style={{ marginBottom: '8px' }}>
