  ...
  return (
    <>
      ...
      <Grid container>
        <Grid item sm={12} md={6}>
          <Grid
            sx={{
              ...
              color: 'white',
              height: `calc(100vh - ${ds.space.mul(1, 5)} - ${ds.space.mul(1, 5)} - ${ds.space[1]})`,
          ...
          >
            ...
            {!branding.loading && (
              <SafeIcon
                ...
                style={{ maxWidth: ds.space.mul(0, 108), maxHeight: ds.space.mul(0, 38), objectFit: 'contain', margin: `${ds.space[6]} 0px` }}
              />
            )}
            <Swiper
              effect={'fade'}
        ...
        <Grid item sm={12} md={6} height={{ md: '100vh', sm: 'auto' }} flexGrow={1} p={ds.space.mul(1, 5)} position={'relative'}>
          <Box
            display='flex'
            alignItems='center'
            justifyContent='center'
            ...
            margin={'auto'}
          >
            <Box mt={ds.space[6]} textAlign='center'>
              {children}
            </Box>
          </Box>
        </Grid>
...
export default function RegisterVerify(): React.ReactElement {
  ...
  const messageMap: Record<string, React.ReactElement> = {
    'invalid token': (
      <Typography fontSize={'var(--ds-text-heading)'}>
    ...
    'internal error': (
      <Typography fontSize={'var(--ds-text-heading)'}>Unable to validate Email, Please connect with support or try again after some time.</Typography>
    ),
  };

  const getVerificationMessageComponent = (message: string): React.ReactElement => {
    message = message.toLowerCase();
    return messageMap[message] || <Typography fontSize={'var(--ds-text-heading)'}>{message}</Typography>;
  };
  ...
  return (
    <BaseTemplate>
      <br />
      <br />
      <Typography textAlign={'center'} fontSize={'var(--ds-text-display)'} fontWeight={'600'} color={ds.gray[700]}>
