...
function sortAvailableCards(cards, criticalCards, highCards, infoCards) {
  const uniqueCriticalCards = [...new Set(criticalCards)];
  const uniqueHighCards = [...new Set(highCards)].filter((card) => !uniqueCriticalCards.includes(card));
  const uniqueInfoCards = [...new Set(infoCards)].filter((card) => !uniqueCriticalCards.includes(card) && !uniqueHighCards.includes(card));

  return [...cards].sort((A, B) => {
    const cardA = new A();
    const cardB = new B();
    const getPriority = (cardId) => {
      if (uniqueCriticalCards.includes(cardId)) return 3;
      if (uniqueHighCards.includes(cardId)) return 2;
      if (uniqueInfoCards.includes(cardId)) return 1;
      return 0;
    };
    return getPriority(cardB.id) - getPriority(cardA.id);
  });
}
