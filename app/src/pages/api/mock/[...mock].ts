...
import autoPilot from './json/autoPilot.json';
import autoPlaybook from './json/autoPlaybook.json';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    ...
    if (mockQuery === 'autoPilot') {
      return res.status(200).json(autoPilot);
    } else if (mockQuery === 'autoPlaybook') {
      return res.status(200).json(autoPlaybook);
    } else if (mockQuery === 'home') {
