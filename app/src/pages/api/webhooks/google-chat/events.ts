...
export default async function trigger(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // non-JSON body surfaces as "unable to process your request" on the
    // client. Reject error envelopes so an upstream failure doesn't reach
    // Google as a "valid" empty reply; fall back to {} for arrays / non-objects
    // / older builds that responded with plain "ok".
    const isPlainObject = response.data && typeof response.data === 'object' && !Array.isArray(response.data);
    if (isPlainObject && (response.data.error || response.data.errors)) {
      throw new Error(response.data.error || JSON.stringify(response.data.errors));
    }
    const body = isPlainObject ? response.data : {};
