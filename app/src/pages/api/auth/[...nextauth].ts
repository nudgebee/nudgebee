...
import { encodeHasuraJWT, decodeHasuraJWT } from '@lib/internal';
import {
...
export interface NudgebeeLicenseDetails {
  licenseType: string;
  tenantId?: string;
  email?: string;
}

let _cachedLicenseDetails: { details: NudgebeeLicenseDetails; expiresAt: number } | null = null;
...
async function getLicenseDetails(): Promise<NudgebeeLicenseDetails> {
  const currentTimeSec = Math.floor(Date.now() / 1000);
  if (_cachedLicenseDetails && _cachedLicenseDetails.expiresAt > currentTimeSec) {
    return _cachedLicenseDetails.details;
  }
  // License service integration planned for future release
  ...
  if (license) {
    ...
    try {
      ...
      if (result.payload.exp) {
        ...
        if (result.payload.exp < currentTime) {
      ...
      console.log('unable to varify license', e);
      throw e;
    }
    try {
      const decoded = decodeJwt(license);
      const details: NudgebeeLicenseDetails = {
      ...
      console.log('unable to decode jwt', e);
      throw e;
    }
  }
  const details: NudgebeeLicenseDetails = { licenseType: licenseType, tenantId: tenantId };
...
async function jwtGenerateHasuraTokenAndUpdate(token: any) {
  ...
  if (token.hasuraIdToken) {
    try {
      const decodedToken = await decodeHasuraJWT(token.hasuraIdToken as string);
  ...
  token.hasuraIdToken = await encodeHasuraJWT(
    {
      id: token.id,
      roles: token.roles,
      tenant: token.tenant,
