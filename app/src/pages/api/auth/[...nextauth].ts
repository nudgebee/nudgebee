...
/* encodeSessionJWT/decodeSessionJWT no longer needed here — the inner
 * NextAuth-session JWT (hasuraIdToken) was Hasura-era leftover. NextAuth's
 * own session token is the authoritative session credential for the browser
 * flow; nbctl/automation still mint+verify via /api/auth/token →
 * rpcGateway.synthesizeJwtFromBearer. */
import {
