...
const updateTokenQuery = `mutation UpdateToken($id: String!, $status: String!, $updated_at: String!){
  signup_update_status(id: $id, status: $status, updated_at: $updated_at) {
    id
  }
}`;

const query = `query GetByToken($token: String!){
  signup_check_token(token: $token) {
    id
    verification_status
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Self-signup verification is only allowed on SaaS-tier deployments;
  // gate on canonical license tier from services-server, not env-var
  // heuristic. See app/src/pages/api/auth/signup.ts for rationale.
  ...
  if (response.data.data.signup_check_token.length === 0) {
    res.status(400).json({
      message: 'Invalid token',
    });
    return;
  ...
  const tokenDetails = response.data.data.signup_check_token[0];
