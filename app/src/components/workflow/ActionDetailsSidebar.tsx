...
const ActionDetailsSidebar: React.FC<ActionDetailsSidebarProps> = ({
  ...
  const renderDynamicForm = (schema: any, _title: string, description: string) => {
    ...
    const renderField = (fieldName: string, fieldSchema: SchemaProperty) => {
      ...
      // Foreach sub-task builder replaces the raw-JSON tasks field. Other
      ...
      // Determine field type based on schema (shared with the foreach sub-task form)
      const fieldType = resolveFieldType(fieldName, fieldSchema, { hasResourceNameField });
