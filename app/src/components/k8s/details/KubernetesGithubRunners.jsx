 ...
 * https://grafana.com/grafana/dashboards/25015 (Runner Pool → Job Throughput →
 * Controller Health), but the expressions are mapped onto the metrics ARC
 * actually exports here: 25015 is written for the ARC v2 runner-scale-set
 * listener (`gha_*`), while this deployment runs the RunnerDeployment /
 * HorizontalRunnerAutoscaler flavour (`horizontalrunnerautoscaler_*`,
 * `runnerdeployment_*`, `github_rate_limit*`).
 *
 * The controller must be scraped by the cluster's metrics provider for this tab
 * to have data — enable `metrics.serviceMonitor`/`metrics.proxy` on the ARC
 * chart and point a scrape config at the controller's `metrics-port`.
 */
...
const SECTIONS = ['Runner Pool', 'Job Throughput', 'Controller Health'];
...
const seriesLabel = (target, metric) => {
  ...
  // `reconcile-rate` groups by (controller, result) but legends on controller —
  // append the result so success/error lines stay tellable apart.
  const suffix = metric?.result ? ` (${metric.result})` : '';
