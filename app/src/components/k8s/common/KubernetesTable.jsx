      ...
      },
      {
        component: (
          <DsButton
            ...
            href={`/investigate?id=${event.id}&accountId=${event.account_id}`}
  ...
  return (
    <ListingLayout sx={{ mb: 0, pt: ds.space[4] }}>
      <ListingLayout.Body>
        ...
        <CustomTable tableData={tableData} headers={headers} rowsPerPage={10} tableHeadingCenter={['Severity', 'Classification', 'Alert Status']} />
      </ListingLayout.Body>
    </ListingLayout>
  );
};
    ...
    <ListingLayout id='box-cost-charts'>
      ...
      <ListingLayout.Body>
        <Grid
          ...
          sx={{
            borderRadius: '0 0 var(--ds-radius-lg) var(--ds-radius-lg)',
            boxShadow: '0px var(--ds-space-1) calc(var(--ds-space-1) * 5) 0px color-mix(in srgb, var(--ds-gray-500) 25%, transparent)',
        ...
        >
          <Grid item md={12}>
            <Chart.Line
              colors={resolveColors([ds.purple[500], 'gray'])}
              labels={costData.labels}
              dataset={[
                {
                  borderColor: resolveColor(ds.purple[500]),
...
const KubernetesRelatedEventTable = ({ query, tableName }) => {
  ...
  if (evidencedata) {
    ...
    if (filterdata && filterdata.length > 0) {
      ...
      return <CustomTable headers={filterdata[0].data.headers} tableData={convertedJson2} rowsPerPage={convertedJson2.length} />;
    }
  }
...
export const KubernetesUtilizationCharts3 = ({ accountId, query }) => {
  return (
    <>
      ...
      />
      <Heading value={'Request & Latency'} borderWidth='md' borderColor={ds.blue[500]} />
      <Box mb={ds.space[2]} />
