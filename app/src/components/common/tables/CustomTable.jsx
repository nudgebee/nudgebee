/**
 * CustomTable — domain swiss-army-knife table for the existing app.
 *
 * (white header + brand-navy delimiter, top/bottom-only container borders, top-aligned cells,\
 *  1px row dividers, skeleton loading state). It is the primary table for the
...
const buildResizeTableSx = ({ totalTableWidth, isResizing, stickyFirstColumn }) => ({
  ...
  'thead tr th:last-of-type': {
    ...
    right: 0,
    zIndex: 3,
    // Match the header fill so the sticky last column doesn't expose a white
    // strip while other columns scroll underneath.
    background: `color-mix(in srgb, ${ds.teal[50]} 50%, var(--ds-background-100))`,
  },
...
const CustomTable = ({
  ...
  // Columns flagged `mandatory: true` are always visible and cannot be deselected in the column selector.
  const isMandatoryColumn = useCallback((h) => typeof h === 'object' && h !== null && h.mandatory === true, []);

  const deriveVisibleIndices = useCallback(
    (hdrs) => {
      if (!hasDefaultVisibleConfig) return hdrs.map((_, i) => i);
      return hdrs.map((h, i) => (typeof h === 'string' || h.defaultVisible === true || isMandatoryColumn(h) ? i : null)).filter((i) => i !== null);
    },
    [hasDefaultVisibleConfig, isMandatoryColumn]
  );
  ...
  const handleColumnToggle = useCallback(
    (idx) => {
      if (isMandatoryColumn(headers[idx])) return;
  ...
  const renderSkeletonBody = () => {
    const skeletonRowCount = Math.min(rowsPerPage || 5, stickyHeader ? 5 : 10);
    return (
      <TableBody
        sx={
          isExpandableRows
  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          >
            ...
            <TableRow
              sx={{
                // Redesigned Table header: a flat tinted bar (no radius) spanning
                // the table width, bounded by a top + bottom rule only. The bottom
                // rule is one step darker than the top so the header reads as
                // seated on the body rather than floating.
                th: {
                  ...
                  borderBottom: 'none',
                  boxShadow: `inset 0 1px 0 0 ${ds.teal[100]}, inset 0 -1px 0 0 ${ds.teal[200]}`,
                },
              }}
            >
              {filteredHeaders.map((head, idx) => {
      ...
      {showColumnSelector && columnSelectorOpen && (
        <Popover
          open={true}
        ...
        >
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 'var(--ds-space-2)' }}>
            {headers.map((head, idx) => {
              ...
              const mandatory = isMandatoryColumn(head);
              return (
                <Box key={idx} sx={{ display: 'flex', alignItems: 'center' }}>
                  <Checkbox
                    checked={mandatory || visibleColumnIndices.includes(idx)}
                    disabled={mandatory}
                    onChange={() => handleColumnToggle(idx)}
                    label={capitalize(headerName)}
                    size='sm'
                  />
                </Box>
              );
