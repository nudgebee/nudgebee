...
const KubernetesRightSizingPopupForm = ({
  ...
  // Event-sourced Raise PR replays the code fix stored by the event's log
  ...
  const eventAnalysisReady = recommendationSource !== 'event' || (eventAnalysisCompleted && eventAnalysisHasDiff);
          ...
          <>
            ...
            {recommendationSource === 'event' ? (
              eventAnalysisReady ? (
                ...
                <Typography
                  data-testid='raise-pr-analysis-gate'
                  sx={{ mt: 'var(--ds-space-4)', mb: 'var(--ds-space-5)', fontSize: 'var(--ds-text-body-lg)', color: ds.amber[700] }}
                >
                  {eventAnalysisFailed
                    ? 'Log analysis failed — please re-run the analysis to generate a fix.'
                    : !eventAnalysisCompleted
                    ? 'Log analysis is still running — Raise PR will be available once it completes.'
                    : 'No code fix was generated by the analysis; re-run the analysis to raise a PR.'}
                </Typography>
              )
                ...
                <Grid item>
                  <CustomButton
                    ...
                    disabled={
                      ...
                      (recommendationSource !== 'event' && !Object.keys(selectedWorkloadAnnotations).length) ||
                      !eventAnalysisReady ||
                      loading
                    }
                    text='Save'
