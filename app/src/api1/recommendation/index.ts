  ...
  async listRecommendationNamesapces({
    ...
    if (
      ...
      query list_namespaces {
        recommendation: recommendation_groupings_v2(where: {account_id: {_eq: "${accountId}"}, category: {_eq: "${category}"}, status: {_in: ["${status}"]}${ruleFilter}}, column_transformations: [{expr: "distinct", name: "resource_k8s_namespace"}]) {
          rows {
            resource_k8s_namespace
          }
        }
    ...
    `;
      ...
      const NAMESAPCES = `
      query list_namespaces {
        recommendation: recommendation_security_groupings_v2(where: {account_id:{_eq:"${accountId}"},status:{_in:["${status}"]}}, column_transformations: [{expr: "distinct", name: "namespace"}]) {
          rows{
            namespace
          }
        }
    ...
    `;
      ...
      const NAMESAPCES = `
      query list_namespaces($accountId: String!, $status: [String!]!${withRuleName ? ', $ruleName: String!' : ''}) {
        recommendation: recommendations_v2(where: {
          account_id: {_eq: $accountId}, 
          category: {_eq: "Configuration"}, 
          status: {_in: $status}${withRuleName ? ', rule_name: {_eq: $ruleName}' : ''}
        }) {
          rows{
            resource_k8s_namespace
            recommendation
    ...
    `;
      const variables: { accountId: string; status: string[]; ruleName?: string | string[] } = {
        accountId,
        status: [status],
      };
      if (withRuleName) {
