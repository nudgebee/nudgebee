...
import apiAskNudgebee from '@api1/ask-nudgebee'; // Adjust path
  ...
  const fetchConversation = useCallback(
    async (sessionId, conversationId, source, isNewChat) => {
      ...
      try {
        ...
        // Use lightweight polling query for poll requests (no heavy TOAST columns)
        // BUT use full query when conversation is WAITING — the lightweight derived table
        // may not return newly created followup messages (backend caching/replication lag)
        const useFullQueryForPoll = source === 'poll' && conversationStatusRef.current === 'WAITING';
        const fetchFn = source === 'poll' && !useFullQueryForPoll ? apiAskNudgebee.getLlmConversationPolling : apiAskNudgebee.getLlmConversation;
          ...
          if (!isNewChat && !isStale()) {
            dispatch({
              ...
              payload: (prev) => {
                ...
                // Preserve response text from previous state when lightweight polling returns empty
                const prevResponseTexts = {};
                prev.forEach((m) => {
                  if (m.type === 'response' && m.text && m.id) {
                    prevResponseTexts[m.id] = m.text;
                  }
                });
                const merged = allMessages.map((msg) => {
                  if (msg.type === 'response' && !msg.text && prevResponseTexts[msg.id]) {
                    return { ...msg, text: prevResponseTexts[msg.id] };
                  }
                  return msg;
                });
            ...
            if (isTerminalStatus || isWaitingStatus) {
              ...
              // Poll used lightweight query — re-fetch with full detail for terminal/waiting response
              if (source === 'poll' && !isStale()) {
                const fullRes = await apiAskNudgebee.getLlmConversation({ accountId, sessionId, conversationId });
                if (!isStale()) {
                  const fullConversations = fullRes?.data?.data?.llm_conversations ?? [];
                  if (fullConversations.length > 0) {
                    const fullResponse = fullConversations[fullConversations.length - 1];
                    const { allMessages: fullMessages } = parseConversationMessages(fullResponse.llm_conversation_messages, accountId);
                    dispatch({
                      type: 'SET_MESSAGES',
                      payload: (prev) => {
                        const optimistic = prev.filter((m) => m.isOptimistic);
                        return [...fullMessages, ...optimistic];
                      },
                    });
                  }
                }
