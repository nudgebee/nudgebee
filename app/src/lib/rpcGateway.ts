...
// tag style. The account-ids family (previously Hasura RLS inputs) has no
// consumers post-Hasura; only the 4 fields below are needed.
...
export async function forwardAction(opts: ForwardOptions): Promise<ForwardResult> {
  ...
  // the action permits, not just the active/default role — Hasura's gate
