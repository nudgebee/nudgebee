...
async function getLicenseDetails(): Promise<LicenseDetails> {
  ...
  if (license) {
    ...
    try {
      ...
      console.log('unable to verify license', e);
      throw e;
    }
    try {
      const decoded = decodeJwt(license);
      return { licenseType: decoded.licenseType as string, tenantId: decoded.tenantId as string, email: decoded.email as string };
    } catch (e) {
      console.log('unable to decode jwt', e);
      throw e;
    }
  }
  return { licenseType: licenseType, tenantId: tenantId };
}
