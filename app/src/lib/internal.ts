...
export async function encodeSessionJWT(user: any, claims: any, exp: number, iat: number): Promise<string> {
  ...
  if (user) {
    ...
    // Session claims at JWT root. Earlier versions wrapped these inside the
    // `https://hasura.io/jwt/claims` namespace (a Hasura-JWT-mode convention);
    // with Hasura gone, the namespace is dead weight. Decoders accept both
    // the new flat shape and the legacy wrapped shape for in-flight tokens.
