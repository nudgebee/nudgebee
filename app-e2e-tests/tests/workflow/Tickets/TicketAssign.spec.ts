...
test("Automation workflow Ticket Assign", async ({ page }) => {
  ...
  await page.getByRole("textbox", { name: "Ticket ID" }).fill("405");
  ...
  await page.getByRole("textbox", { name: "Assignee" }).fill("VanshikaR7");
  ...
  await runWorkflowWithGraphQLValidation(page, locators, "Automation-> Action-> Ticket Assign");
});
