...
func (s *IntegrationTestSuite) TestSwitchWorkflow() {
	...
	// 2. Execute with value 'A'
	runID_A, err := s.executeWorkflow(createdWorkflow.ID, map[string]any{"value": "A"})
	s.Require().NoError(err, "Failed to execute workflow for A")

	temporalId_A, err := s.resolveTemporalWorkflowID(context.Background(), createdWorkflow.ID, runID_A)
	s.Require().NoError(err)
	workflowExecution_A := s.temporalClient.GetWorkflow(context.Background(), temporalId_A, runID_A)
	var result_A string
	err = workflowExecution_A.Get(context.Background(), &result_A)
	s.Require().NoError(err, "Workflow A execution failed")

	// Verify task-a executed
	executionDetails_A := s.getWorkflowExecutionDetails(createdWorkflow.ID, runID_A)
	var taskAFound bool
	for _, t := range executionDetails_A.Tasks {
		if t.ID == "task-a" {
			taskAFound = true
	...
	s.Assert().True(taskAFound, "Task A should have been executed")

	// 3. Execute with value 'B'
	runID_B, err := s.executeWorkflow(createdWorkflow.ID, map[string]any{"value": "B"})
	s.Require().NoError(err, "Failed to execute workflow for B")

	temporalId_B, err := s.resolveTemporalWorkflowID(context.Background(), createdWorkflow.ID, runID_B)
	s.Require().NoError(err)
	workflowExecution_B := s.temporalClient.GetWorkflow(context.Background(), temporalId_B, runID_B)
	var result_B string
	err = workflowExecution_B.Get(context.Background(), &result_B)
	s.Require().NoError(err, "Workflow B execution failed")

	executionDetails_B := s.getWorkflowExecutionDetails(createdWorkflow.ID, runID_B)
	var taskBFound bool
	for _, t := range executionDetails_B.Tasks {
		if t.ID == "task-b" {
			taskBFound = true
			s.Assert().Equal(model.TaskStatusCompleted, t.Status)
			break
	...
	s.Assert().True(taskBFound, "Task B should have been executed")

	// 4. Execute with value 'C' (Default)
	runID_C, err := s.executeWorkflow(createdWorkflow.ID, map[string]any{"value": "C"})
	s.Require().NoError(err, "Failed to execute workflow for C")

	temporalId_C, err := s.resolveTemporalWorkflowID(context.Background(), createdWorkflow.ID, runID_C)
	s.Require().NoError(err)
	workflowExecution_C := s.temporalClient.GetWorkflow(context.Background(), temporalId_C, runID_C)
	var result_C string
	err = workflowExecution_C.Get(context.Background(), &result_C)
	s.Require().NoError(err, "Workflow C execution failed")

	executionDetails_C := s.getWorkflowExecutionDetails(createdWorkflow.ID, runID_C)
	var taskCFound bool
	for _, t := range executionDetails_C.Tasks {
		if t.ID == "task-c" {
			taskCFound = true
			s.Assert().Equal(model.TaskStatusCompleted, t.Status)
			break
	...
	s.Assert().True(taskCFound, "Task C should have been executed")
}
...
func (s *IntegrationTestSuite) TestSwitchNestedWorkflow() {
	...
	// Check if task-nested-a was executed
	var nestedTaskFound bool
	var foundIDs []string
	for _, t := range details_Nested.Tasks {
		foundIDs = append(foundIDs, t.ID)
		// We look for the ID that indicates it ran inside the nested structure
		// root-switch -> nested-switch -> task-nested-a
		// IDs: "root-switch-nested-switch-task-nested-a"
		if t.ID == "root-switch-nested-switch-task-nested-a" {
			nestedTaskFound = true
			s.Assert().Equal(model.TaskStatusCompleted, t.Status)
		}
	}
	s.T().Logf("Found Task IDs: %v", foundIDs)
	s.Assert().True(nestedTaskFound, "Nested task should have executed within the switch scope")

	// 2. Dependency Chain Path
	...
	for _, t := range details_Chain.Tasks {
		if t.ID == "root-switch-task-chain-1" {
			chain1Found = true
			s.Assert().Equal(model.TaskStatusCompleted, t.Status)
		}
		if t.ID == "root-switch-task-chain-2" {
			chain2Found = true
			s.Assert().Equal(model.TaskStatusCompleted, t.Status)
		}
	}
	s.Assert().True(chain1Found, "Chain task 1 should have executed in switch scope")
	s.Assert().True(chain2Found, "Chain task 2 should have executed in switch scope")
