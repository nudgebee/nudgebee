...
func (t *SwitchTask) Execute(taskCtx types.TaskContext, params map[string]any) (result any, err error) {
	taskCtx.GetLogger().Warn("SwitchTask.Execute called - this is unexpected in the new architecture as switch tasks should be handled inline by the executor.")
	return nil, fmt.Errorf("SwitchTask.Execute is deprecated and should not be called directly")
}
