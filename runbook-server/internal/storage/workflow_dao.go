...
func (s *WorkflowDao) Save(ctx context.Context, tenantID, accountID string, wf model.Workflow) (string, error) {
	...
	query := `
		INSERT INTO workflows (id, tenant_id, account_id, name, definition, tags, status, created_by, updated_by)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
	`
	_, err = tx.ExecContext(ctx, query, id, tenantID, accountID, wf.Name, wfBytes, tagBytes, wf.Status, wf.CreatedBy, wf.UpdatedBy)
	if err != nil {
...
func (s *WorkflowDao) List(ctx context.Context, tenantID, accountID string, request model.ListWorkflowRequest) ([]model.Workflow, int, error) {
	...
	mainQuery := `
		SELECT w.id::text, w.name, w.definition, w.tags, w.status, w.last_execution_status, w.last_execution_status_message, w.last_execution_time, w.created_by, w.updated_by, w.created_at, w.updated_at,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&wfID, &wfName, &wfBytes, &tagBytes, &status, &lastExecutionStatus, &lastExecutionStatusMessage, &lastExecutionTime, &createdBy, &updatedBy, &createdAt, &updatedAt,
...
func (s *WorkflowDao) Find(ctx context.Context, tenantID, accountID string, id string) (*model.Workflow, error) {
	...
	query := `
		SELECT id::text, name, definition, tags, status, last_execution_status, last_execution_status_message, last_execution_time, created_by, updated_by, created_at, updated_at FROM workflows
		WHERE tenant_id = $1 AND account_id = $2 AND id = $3
	`
	err := s.db.QueryRowContext(ctx, query, tenantID, accountID, id).Scan(&wfID, &wfName, &wfBytes, &tagBytes, &status, &lastExecutionStatus, &lastExecutionStatusMessage, &lastExecutionTime, &createdBy, &updatedBy, &createdAt, &updatedAt)
	if err != nil {
...
func (s *WorkflowDao) FindByName(ctx context.Context, tenantID, accountID, name string) (*model.Workflow, error) {
	...
	`
	err := s.db.QueryRowContext(ctx, query, tenantID, accountID, name).Scan(&id, &wfName, &wfBytes, &tagBytes, &status, &lastExecutionStatus, &lastExecutionStatusMessage, &lastExecutionTime, &createdBy, &updatedBy, &createdAt, &updatedAt)
	if err != nil {
