...
// FetchWorkflowConfigsActivity fetches all configs and secrets for a given account.
// It returns a struct containing configs (plain) and secrets (decrypted).
func (a *WorkflowActivities) FetchWorkflowConfigsActivity(ctx context.Context, tenantID, accountID string) (*FetchConfigsResponse, error) {
	configs, err := a.ConfigService.ListConfigsDecrypted(ctx, tenantID, accountID, nil)
	if err != nil {
		return nil, err
	}
