...
// This file mirrors api-server/services/audit/redact.go. runbook-server is a
// separate Go module writing to the same shared `audit` table (see CreateAudit),
// so the redactor is duplicated here rather than imported cross-module — the same
// rationale under which valueOrNil is duplicated across the audit writers. Keep
// the two in sync.
