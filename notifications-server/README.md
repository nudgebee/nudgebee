...
Configure the Slack App's redirect URL to `${BASE_URL}/api/integrations/slack/callback` and the event subscription URL to `${BASE_URL}/api/webhooks/slack/events`.
...
Register the Azure AD app as a bot in Azure Bot Service, add it to your Teams app manifest, and point the Bot Service messaging endpoint at `${BASE_URL}/api/webhooks/msteams/events`.
...
Configure the Google Cloud project's OAuth redirect to `${BASE_URL}/api/integrations/google/callback` and the Chat app's event endpoint to `${BASE_URL}/api/webhooks/google-chat/events`.
...
OAuth-install flows (Slack, MS Teams, Google Chat) require the platform to be able to reach **your** dev environment's callback URL — `localhost` won't work. Use a tunnel:

```bash
# Option A: ngrok
ngrok http 3000           # the Next.js frontend on :3000 — handles /api/integrations/<provider>/callback
...
```bash
BASE_URL=https://<your-tunnel>.ngrok.app
```

Update the provider-side app configuration to match:

| Provider     | Redirect / event URL to register with the provider                          |
|--------------|------------------------------------------------------------------------------|
| Slack        | OAuth redirect: `${BASE_URL}/api/integrations/slack/callback` <br> Event subscription: `${BASE_URL}/api/webhooks/slack/events` |
| MS Teams     | OAuth redirect: `${BASE_URL}/api/integrations/ms-teams/callback` <br> Bot messaging endpoint: `${BASE_URL}/api/webhooks/msteams/events` |
| Google Chat  | OAuth redirect: `${BASE_URL}/api/integrations/google/callback` <br> Chat app event URL: `${BASE_URL}/api/webhooks/google-chat/events` |

The Next.js frontend handles the user-facing callback paths and proxies to notifications-server's internal Bolt / OAuth handlers — see the [root CLAUDE.md](../CLAUDE.md) on the `/api/integrations/<provider>/*` surface.

## Troubleshooting

**OAuth redirect mismatch** — Provider returns `redirect_uri_mismatch` after the user authorizes. The URL registered in the provider's app config must match `BASE_URL` exactly (scheme, host, port, no trailing slash). Re-check after rotating tunnel URLs.

**Slack webhook signature failures** — `SLACK_SIGNING_SECRET` must match the *Signing Secret* shown in the Slack App's *Basic Information* page, not the *Verification Token* (legacy). The signing secret rotates if the app is regenerated.

**MS Teams bot not receiving messages** — Bot must be registered in Azure Bot Service AND added to the Teams app manifest. The `MS_TEAMS_BOT_ENDPOINT` env must point at `${BASE_URL}/api/webhooks/msteams/events`. Check the Bot Service "Channels" tab for delivery diagnostics.

**Google Chat events not arriving** — Chat-app event URL is configured per-project in the Google Cloud console; verifying it requires returning a `200 OK` to Google's verification ping at the URL. Confirm the consumer is running (RabbitMQ + DB up) before the URL verification.

**Notifications stuck in the queue** — Check the consumer is running (`docker logs notifications-server` or the pod logs). Failed messages go to `notifications_delayed` and retry after `RABBITMQ_DEAD_LETTER_DELAY_MS`; if they keep landing there, the message body is malformed or the platform credentials are wrong.

**Emails not sent** — `EMAIL_SERVER_PASSWORD` cleartext leaks happen via misconfigured SMTP debug logging. Verify `EMAIL_SERVER_HOST` resolves, port `EMAIL_SERVER_PORT` is open from the pod (`kubectl exec -- nc -zv host port`), and the SMTP server accepts `EMAIL_FROM` as a sender (some relays reject unverified domains).
