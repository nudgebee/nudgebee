...
# Default URL constant to avoid duplication
DEFAULT_BASE_URL = "https://nudgebee.com"
...
class URLSettings(BaseSettings):
    ...
    # Base application URL (where users are redirected)
    base_url: str = Field(DEFAULT_BASE_URL, validation_alias=AliasChoices("BASE_URL", "base_url"))

    # Branding (used in notification headers/footers)
    branding_name: str = Field(
        "Nudgebee",
        validation_alias=AliasChoices("BRANDING_NAME", "branding_name"),
    )
    # Accepts full URL (https://...) or relative path (/branding/calsoft/logo.svg).
    # Relative paths are resolved to absolute URLs using base_url.
    branding_logo_url: str = Field(
        "https://s3.amazonaws.com/nudgebee-documents-v2/images/NudgebeeDark.png",
        validation_alias=AliasChoices("BRANDING_LOGO_URL", "branding_logo_url"),
    )
    branding_support_email: str = Field(
        "nudgebee.support@nudgebee.com",
        validation_alias=AliasChoices("BRANDING_SUPPORT_EMAIL", "branding_support_email"),
    )
    branding_onboarding_image_url: str = Field(
        "https://s3.amazonaws.com/nudgebee-documents-v2/images/Onboarding.png",
        validation_alias=AliasChoices("BRANDING_ONBOARDING_IMAGE_URL", "branding_onboarding_image_url"),
    )
    branding_footer_bg_color: str = Field(
        "#10264c", validation_alias=AliasChoices("BRANDING_FOOTER_BG_COLOR", "branding_footer_bg_color")
    )
    branding_footer_link_color: str = Field(
        "#ffcc00", validation_alias=AliasChoices("BRANDING_FOOTER_LINK_COLOR", "branding_footer_link_color")
    )
    branding_primary_color: str = Field(
        "#facf39", validation_alias=AliasChoices("BRANDING_PRIMARY_COLOR", "branding_primary_color")
    )
    branding_header_bg_color: str = Field(
        "#004C74", validation_alias=AliasChoices("BRANDING_HEADER_BG_COLOR", "branding_header_bg_color")
    )
    ...
    def resolve_relative_urls(self):
        ...
        for field_name in ("branding_logo_url", "branding_onboarding_image_url"):
            value = getattr(self, field_name)
            if value.startswith("/"):
                setattr(self, field_name, f"{self.base_url}{value}")
        return self
...
class MSTeamsSettings(BaseSettings):
    ...
    bot_endpoint: str = Field(
        "https://notificationc55fec.azurewebsites.net/api/messages",
...
class EmailSettings(BaseSettings):
    ...
    from_address: str = Field(
        "nudgebee.support@nudgebee.com", validation_alias=AliasChoices("EMAIL_FROM", "email_from")
    )
    ...
    def is_configured(self) -> bool:
        ...
        return bool(self.server_host and self.server_user and self.server_password)
...
class HasuraSettings(BaseSettings):
    """Hasura GraphQL settings."""

    endpoint: str = Field(
        "http://localhost:8080", validation_alias=AliasChoices("HASURA_GRAPHQL_ENDPOINT", "hasura_graphql_endpoint")
    )
    admin_secret: str = Field(
        "", validation_alias=AliasChoices("HASURA_GRAPHQL_ADMIN_SECRET", "hasura_graphql_admin_secret")
    )
    ...
    def graphql_url(self) -> str:
        ...
        return f"{self.endpoint}/v1/graphql"
...
class Settings(BaseSettings):
    ...
    hasura: HasuraSettings = Field(default_factory=HasuraSettings)
