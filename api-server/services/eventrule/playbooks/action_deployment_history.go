...
func (a *deploymentHistoryAction) AutoExecute(ctx PlaybookActionContext) (PlaybookActionResponse, error) {
	...
	// Step 1: Try using subject_name directly as rollout name
	...
	if (err != nil || len(deployments) == 0) && ctx.GetEvent().Labels["service"] != "" {
		slog.Info("No deployments found with subject_name directly, trying fallback",
			"subject_name", subjectName, "account_id", accountId)

		foundRolloutName, foundNamespace, fallbackErr := findRolloutNameFromWorkload(accountId, ctx.GetEvent().Labels["service"])
