...
func (s *DatadogMetricSource) FetchMetricsQuery(
	...
	for queryKey, rawQuery := range fetchMetricsRequest.Queries {
		encodedQuery := url.QueryEscape(injectDatadogLabels(rawQuery, fetchMetricsRequest.Labels))
...
// injectDatadogLabels appends tag filters to a Datadog metric query using {tag:value} syntax.
func injectDatadogLabels(expr string, labels map[string]string) string {
	if len(labels) == 0 {
		return expr
	}
	...
	if idx := strings.Index(expr, "{"); idx != -1 {
		...
		if existing == "" {
			return expr[:idx+1] + selector + expr[closeIdx:]
		}
		return expr[:idx+1] + existing + "," + selector + expr[closeIdx:]
	}
...
func (s *DatadogMetricSource) GetQuery(_ *security.RequestContext, req FetchMetricsRequest) (string, error) {
	for _, q := range req.Queries {
		return injectDatadogLabels(q, req.Labels), nil
	}
	return "", nil
}
