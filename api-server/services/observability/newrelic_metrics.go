...
func (s *NewRelicMetricSource) GetQuery(_ *security.RequestContext, req FetchMetricsRequest) (string, error) {
	...
	for _, rawQuery := range req.Queries {
		...
		return s.buildNRQLMetricQuery(rawQuery, startTime, endTime, stepInterval, req.Instant, req.Labels), nil
	}
	return "", nil
}
...
func (s *NewRelicMetricSource) FetchMetricsQuery(ctx *security.RequestContext, req FetchMetricsRequest) (OutputMetricQuery, error) {
	...
	for queryKey, rawQuery := range req.Queries {
		...
		nrqlQuery := s.buildNRQLMetricQuery(rawQuery, startTime, endTime, stepInterval, req.Instant, req.Labels)
...
// buildNRQLMetricQuery constructs an NRQL query for metrics
func (s *NewRelicMetricSource) buildNRQLMetricQuery(rawQuery string, startTime, endTime int64, stepInterval int, instant bool, labels map[string]string) string {
	...
	if instant {
		return fmt.Sprintf("SELECT average(%s) %s SINCE %d UNTIL %d",
			escapedMetric, where, startTime, endTime)
	}
	return fmt.Sprintf("SELECT average(%s) %s SINCE %d UNTIL %d TIMESERIES %d seconds",
		escapedMetric, where, startTime, endTime, stepInterval)
}
