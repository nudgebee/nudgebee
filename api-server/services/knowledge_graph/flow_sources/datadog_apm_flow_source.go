...
func (s *DatadogAPMFlowSource) BuildFlowRelationships(reqCtx *security.RequestContext, req *core.FlowSourceBuildRequest) ([]*core.DbEdge, []*core.DbNode, error) {
	...
	for _, apmEdge := range graphData.Edges {
		...
		sourceNode, sourceErr := s.matchEntityToNode(sourceEntity, sourceName, sourceKind)
		destNode, destErr := s.matchEntityToNode(destEntity, destName, destKind)
...
func (s *DatadogAPMFlowSource) getDatadogConfig(
	...
	// Create a minimal security context for the integration call
	// In production, this should be properly constructed with user permissions
	secCtx := security.NewRequestContextForSuperAdmin(s.logger, nil, nil)
...
func (s *DatadogAPMFlowSource) matchEntityToNode(
	...
	// Strategy 1: Exact match by service name
	if serviceName, ok := entity.Attributes.IDTags["service"]; ok && serviceName != "" {
		// Try matching by name property
		...
		if err == nil && result.Matched {
			s.logger.Debug("matched entity by service name",
		...
		// Try matching by dd_service_name label (for K8s services)
		result, err = matcher.FindNode(MatchCriteria{
			PropertyMatches: []PropertyMatch{
				{
		...
		if err == nil && result.Matched {
			s.logger.Debug("matched entity by dd_service_name label",
				"dd_service", serviceName,
				"node", result.Node.UniqueKey,
				"confidence", result.Confidence)
			return result.Node, nil
		...
		// Try matching by service_name property
		result, err = matcher.FindNode(MatchCriteria{
			PropertyMatches: []PropertyMatch{
				{
		...
		if err == nil && result.Matched {
			s.logger.Debug("matched entity by service_name property",
				"dd_service", serviceName,
				"node", result.Node.UniqueKey,
				"confidence", result.Confidence)
			return result.Node, nil
	...
	// Strategy 2: Match by entity name (fallback)
