...
func TestGCPSourceShouldIncludeResource(t *testing.T) {
	...
	tests := []struct {
		name     string
		resource sources.CloudResourceRow
		want     bool
	}{
		{
			"allowed compute-engine type",
			sources.CloudResourceRow{Type: "compute-engine", ServiceName: "Compute Engine"},
			true,
		},
		{
			"allowed asset inventory compute type",
			sources.CloudResourceRow{Type: "compute.googleapis.com/Instance", ServiceName: "Compute Engine"},
			true,
		},
		{
			"disallowed type for filtered service",
			sources.CloudResourceRow{Type: "some-unknown-type", ServiceName: "Compute Engine"},
			false,
		},
		{
			"unknown service passes through",
			sources.CloudResourceRow{Type: "custom-thing", ServiceName: "Custom Service"},
			true,
		},
	}
...
func TestCreateSubnetToVPCEdges(t *testing.T) {
	...
	t.Run("single VPC links explicit and infers orphans", func(t *testing.T) {
		...
		lookup := sources.NewNodeLookup([]*core.DbNode{vpc, matched, orphan, absent})
	...
	t.Run("multiple VPCs: no inference for subnets without vpc_id", func(t *testing.T) {
		...
		lookup := sources.NewNodeLookup([]*core.DbNode{vpcA, vpcB, orphan, matched})
...
func TestCreateCDNEdges(t *testing.T) {
	...
	lookup := sources.NewNodeLookup([]*core.DbNode{backendPool, cdnMatched, cdnUnmatched})
...
func TestCreateNodeFromResourceProjectID(t *testing.T) {
	...
	fallback := &sources.CloudResourceRow{
...
func TestCreatePubSubSubscriptionEdges(t *testing.T) {
	...
	lookup := sources.NewNodeLookup([]*core.DbNode{cloudRun, apiRun, appEngine, topic})
...
func TestEnrichExistingURLMapNode(t *testing.T) {
	...
	lookup := sources.NewNodeLookup([]*core.DbNode{existing})
...
func TestShouldSuppressGCPResource_ComputeEngineBilling(t *testing.T) {
	tests := []struct {
		name string
		row  *sources.CloudResourceRow
		want bool
	}{
		{
			"compute-engine billing stub is suppressed",
			&sources.CloudResourceRow{Type: "compute-engine", ServiceName: "Compute Engine", Name: "my-project", Meta: json.RawMessage(`{"nb_source":"billing"}`)},
			true,
		},
		{
			"real compute-engine instance (api source) is kept",
			&sources.CloudResourceRow{Type: "compute-engine", ServiceName: "Compute Engine", Name: "web-1", Meta: json.RawMessage(`{"nb_source":"api","zone":"us-central1-a"}`)},
			false,
		},
		{
			"compute-engine with no meta is kept",
			&sources.CloudResourceRow{Type: "compute-engine", ServiceName: "Compute Engine", Name: "web-1"},
			false,
		},
		{
			"vm-manager stub still suppressed",
			&sources.CloudResourceRow{Type: "vm-manager", Meta: json.RawMessage(`{"nb_source":"billing"}`)},
			true,
		},
		{
			"Cloud Tasks billing rollup is suppressed",
			&sources.CloudResourceRow{Type: "cloud-tasks", ServiceName: "Cloud Tasks", Name: "my-project", Meta: json.RawMessage(`{"nb_source":"billing"}`)},
			true,
		},
		{
			"Secret Manager billing rollup is suppressed",
			&sources.CloudResourceRow{Type: "secret-manager", ServiceName: "Secret Manager", Name: "my-project", Meta: json.RawMessage(`{"nb_source":"billing"}`)},
			true,
		},
		{
			"real api resource (non-billing) is kept",
			&sources.CloudResourceRow{Type: "storage.googleapis.com/Bucket", ServiceName: "Cloud Storage", Name: "my-bucket", Meta: json.RawMessage(`{"nb_source":"api"}`)},
			false,
		},
	}
	for _, tt := range tests {
...
func TestCreateCronJobEdges(t *testing.T) {
	...
	lookup := sources.NewNodeLookup([]*core.DbNode{topic, cloudRun, cronPubsub, cronHTTP, cronExt})
...
func TestCreateRunsAsEdges(t *testing.T) {
	...
	lookup := sources.NewNodeLookup([]*core.DbNode{sa, withSA, noSA})
...
func TestCreateUsesSecretEdges(t *testing.T) {
	...
	lookup := sources.NewNodeLookup([]*core.DbNode{secretA, secretB, svc})
...
func iamBindingRow(role, member string) sources.CloudResourceRow {
...
func TestCreateHasAccessEdges(t *testing.T) {
	...
	lookup := sources.NewNodeLookup([]*core.DbNode{identity, firestore, dataset, table, bucket, topic})
...
func TestCreateHasAccessEdges_FanoutCap(t *testing.T) {
	...
	edges := source.createHasAccessEdges([]sources.CloudResourceRow{iamBindingRow("roles/storage.admin", sa)}, lookup, req)
	if len(edges) != 0 {
...
func TestCreateCacheEndpointEdges(t *testing.T) {
	...
	lookup := sources.NewNodeLookup([]*core.DbNode{cacheA, cacheB, svc, svc2})
...
func TestCreateCacheEndpointEdges_NoCaches(t *testing.T) {
	...
	edges := source.createCacheEndpointEdges(sources.NewNodeLookup([]*core.DbNode{svc}), req)
	if len(edges) != 0 {
