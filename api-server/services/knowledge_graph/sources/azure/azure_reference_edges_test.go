...
func TestCreateReferenceEdges_SkipsUnresolvable(t *testing.T) {
	...
	lookup := sources.NewNodeLookup([]*core.DbNode{arnNode("lb1", core.NodeTypeLoadBalancer, lbArn)})
...
func TestCreateRoleAssignmentEdges(t *testing.T) {
	...
	lookup := sources.NewNodeLookup([]*core.DbNode{identity, kv, sa})

	ra := func(ptype, scope, roleGUID string) sources.CloudResourceRow {
...
func TestCreateRoleAssignmentEdges_SystemAssignedIdentity(t *testing.T) {
	...
	kv := arnNode("kv1", core.NodeTypeSecretVault, kvArn)
	lookup := sources.NewNodeLookup([]*core.DbNode{vm, kv})
...
func TestResolvePrivateEndpointOwnEdges(t *testing.T) {
	...
	lookup := sources.NewNodeLookup([]*core.DbNode{pe, kv, snet})
...
func TestResolveFunctionEdges_AppServicePlan(t *testing.T) {
	...
	lookup := sources.NewNodeLookup([]*core.DbNode{site, plan})
...
func TestCreateAppReferenceEdges(t *testing.T) {
	...
	lookup := sources.NewNodeLookup([]*core.DbNode{site, sa, kv})
