...
func TestAzureSourceRegistered(t *testing.T) {
	if !sources.IsSourceRegistered("azure") {
		t.Error("Azure source not found in global registry")
	}

	registered := sources.ListRegisteredSources()
	found := false
	...
	if !found {
		t.Errorf("Azure not in sources.ListRegisteredSources(): %v", registered)
	}
}
...
func TestAzureShouldIncludeResource(t *testing.T) {
	...
	t.Run("no filter includes all", func(t *testing.T) {
		source, _ := NewAzureSource(AzureSourceConfig{}, logger)
		r := &sources.CloudResourceRow{ServiceName: "Microsoft.Compute", Type: "VirtualMachine"}
	...
	t.Run("filter excludes non-matching type", func(t *testing.T) {
		...
		r := &sources.CloudResourceRow{ServiceName: "Microsoft.Compute", Type: "Disk"}
		if source.shouldIncludeResource(r) {
	...
	t.Run("unfiltered service includes all types", func(t *testing.T) {
		...
		r := &sources.CloudResourceRow{ServiceName: "Microsoft.Network", Type: "VirtualNetwork"}
		if !source.shouldIncludeResource(r) {
...
func TestAzureDefaultFilterDropsComputeArtifacts(t *testing.T) {
	...
	for _, d := range dropped {
		t.Run("drops "+d.serviceName, func(t *testing.T) {
			r := &sources.CloudResourceRow{ServiceName: d.serviceName, Type: d.resType}
			if source.shouldIncludeResource(r) {
	...
	for _, k := range kept {
		t.Run("keeps "+k.serviceName, func(t *testing.T) {
			r := &sources.CloudResourceRow{ServiceName: k.serviceName, Type: k.resType}
			if !source.shouldIncludeResource(r) {
...
func TestAzureConvertResourcesToGraph_Empty(t *testing.T) {
	...
	nodes, edges := helper.ConvertResourcesToGraph(nil, []sources.CloudResourceRow{}, req)
...
func TestAzureShouldSuppressResource(t *testing.T) {
	cases := []struct {
		...
		{"reservation order by type", &sources.CloudResourceRow{Type: "reservationorders", ServiceName: "virtual machines"}, true},
		{"reservation order by service", &sources.CloudResourceRow{Type: "x", ServiceName: "microsoft.capacity/reservationorders"}, true},
		{"capacity service", &sources.CloudResourceRow{Type: "x", ServiceName: "microsoft.capacity/somethingelse"}, true},
		{"nil row", nil, true},
		// Must NOT suppress real resources, incl. billing-sourced ones (e.g. Private Endpoints)
		{"private endpoint (billing-only)", &sources.CloudResourceRow{Type: "privateendpoints", ServiceName: "microsoft.network/privateendpoints"}, false},
		{"flexible server", &sources.CloudResourceRow{Type: "flexibleservers", ServiceName: "microsoft.dbforpostgresql/flexibleservers"}, false},
		{"vm", &sources.CloudResourceRow{Type: "virtualmachines", ServiceName: "microsoft.compute/virtualmachines"}, false},
	}
	for _, c := range cases {
