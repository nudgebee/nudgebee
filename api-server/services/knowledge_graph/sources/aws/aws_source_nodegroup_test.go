...
func TestCreateEKSNodeGroupEdges(t *testing.T) {
	...
	src.metaByType = map[string][]sources.CloudResourceRow{}
	src.metaByTypeAndID = map[string]map[string]sources.CloudResourceRow{
	...
	lookup := sources.NewNodeLookup([]*core.DbNode{ng, cluster, subnetA, subnetB, subnetC})
...
func TestCreateEKSNodeGroupEdges_SkipsNonEKS(t *testing.T) {
	...
	lookup := sources.NewNodeLookup([]*core.DbNode{gke})
...
func TestCreateEKSNodeGroupEdges_NoMetaCache(t *testing.T) {
	...
	lookup := sources.NewNodeLookup([]*core.DbNode{ng})
