...
type DependencyGraph struct {
	...
	// edgeContributors maps a directed edge ("src→dst") to the set of sources
	// that asserted it (k8s, traces, aws, manual, ...). Populated by
	// parseKnowledgeGraphEvidence when the KG evidence carries source /
	// contributing_sources fields; remains nil when evidence is silent on
	// source (e.g. legacy traces_dependency_map evidence). When non-empty,
	// correlation uses it to weight manual-only paths lower so a single
	// user-declared edge cannot single-handedly cross CorrelationScoreThreshold.
	edgeContributors map[string]map[string]bool
}

// observedSourcesForWeighting names the source identifiers that count as
// "observed reality" for the corroboration weight. Any one of these
// asserting an edge upgrades that edge to full weight; a path whose every
// edge is in this set always weights 1.0. Mirror the priority table in
// flow_sources/edge_priority.go — keep in sync when adding new sources.
var observedSourcesForWeighting = map[string]bool{
	"k8s":              true,
	"aws":              true,
	"ebpf":             true,
	"traces":           true,
	"datadog-apm":      true,
	"newrelic-apm":     true,
	"cloud":            true, // CentralizedExternalServiceEnricher edges
	"cloud_enrichment": true,
	"dns_resolver":     true,
	"ip_mapper":        true,
}

// manualOnlySourceWeight is applied to the dependency-distance score when the
// shortest path between two events has no observed-source corroboration —
// every edge along the way was asserted only by the user-declared `manual`
// flow source. 0.7 keeps a distance=1 path scoring 0.40*0.7 + 0.15 causality
// = 0.43, below CorrelationScoreThreshold (0.50): manual-only edges must
// combine with another signal (time proximity, trace co-participation, or a
// corroborating observed edge) to qualify.
const manualOnlySourceWeight = 0.7

// edgeContributorKey returns the canonical key used in DependencyGraph.edgeContributors.
func edgeContributorKey(src, dst string) string {
	return src + "\x00" + dst
}

// recordEdgeContributors stores the set of sources that asserted a given
// directed edge. Called by the KG evidence parser; safe to call repeatedly
// (sources accumulate). The empty-source case is a no-op.
func (g *DependencyGraph) recordEdgeContributors(src, dst string, sources []string) {
	if len(sources) == 0 {
		return
	}
	if g.edgeContributors == nil {
		g.edgeContributors = make(map[string]map[string]bool)
	}
	key := edgeContributorKey(src, dst)
	bucket := g.edgeContributors[key]
	if bucket == nil {
		bucket = make(map[string]bool, len(sources))
		g.edgeContributors[key] = bucket
	}
...
// dependencyHopWeight returns the multiplier to apply to the dependency-
// distance score for the (src,dst) pair. Returns 1.0 (no penalty) when:
//   - the parser didn't populate edge contributors (legacy evidence shape)
//   - the direct edge has any observed-source contributor
//   - the pair has distance > 1 (multi-hop paths already get smaller base
//     contributions and we don't currently walk per-hop sources)
//
// Returns manualOnlySourceWeight when the direct edge is asserted ONLY by
// the manual flow source.
func (g *DependencyGraph) dependencyHopWeight(srcKey, dstKey string, distance int) float64 {
	if g == nil || len(g.edgeContributors) == 0 || distance != 1 {
		return 1.0
	}
	srcKey = g.resolveKey(srcKey)
	dstKey = g.resolveKey(dstKey)
	// Direction-agnostic: a CALLS edge a→b matters for both upstream/downstream
	// scoring, mirroring the bidirectional getDependencyDistance walk.
	for _, key := range []string{
		edgeContributorKey(srcKey, dstKey),
		edgeContributorKey(dstKey, srcKey),
	} {
		bucket, ok := g.edgeContributors[key]
		if !ok {
			continue
		}
		for src := range bucket {
			if observedSourcesForWeighting[src] {
				return 1.0
			}
		}
	}
	return manualOnlySourceWeight
}
...
func parseKnowledgeGraphEvidence(evidence map[string]interface{}) *DependencyGraph {
	...
	for _, edgeRaw := range edgesRaw {
		...
		// Capture which sources asserted this edge so correlation can apply
		// per-source weights. Two payload shapes are supported: the
		// post-V742 first-class fields (source / contributing_sources) and
		// the legacy properties-based fields populated before this
		// migration. Until KG evidence consistently carries either, the
		// resulting contributor map may be empty for older evidence — in
		// that case dependencyHopWeight falls back to 1.0 (no behavior
		// change).
		graph.recordEdgeContributors(sourceKey, destKey, extractEdgeContributors(edge))
	}

	return graph
}

// extractEdgeContributors pulls the source-list from a KG edge payload.
// Priority order:
//  1. edge.contributing_sources (jsonb array of {source, last_seen_at})
//  2. edge.properties.contributing_sources (legacy string array)
//  3. edge.source / edge.properties.created_by_flow_source (single value)
//
// Returning the union avoids penalizing manual edges that any observed
// source also corroborates.
func extractEdgeContributors(edge map[string]interface{}) []string {
	...
	// First-class field (post-migration V742).
	if rawArr, ok := edge["contributing_sources"].([]interface{}); ok {
		for _, item := range rawArr {
			if obj, ok := item.(map[string]interface{}); ok {
				if s, _ := obj["source"].(string); s != "" {
					addUnique(s)
				}
	...
	// Legacy properties slot.
	if props, ok := edge["properties"].(map[string]interface{}); ok {
		if rawArr, ok := props["contributing_sources"].([]interface{}); ok {
			for _, item := range rawArr {
		...
		if v, _ := props["created_by_flow_source"].(string); v != "" {
			addUnique(v)
		}
	}

	// First-class single-source field.
	if v, _ := edge["source"].(string); v != "" {
		addUnique(v)
	}
