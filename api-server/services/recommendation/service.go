...
func GetRecommendation(context *security.RequestContext, id string) (models.Recommendation, error) {
	...
	r := databaseManager.Db.QueryRowx("SELECT * FROM recommendation WHERE id = $1", id)
	if r.Err() != nil {
...
func RetryRecommendationResolution(ctx *security.RequestContext, query RetryRecommendationResolutionRequest) (RetryRecommendationResolutionResponse, error) {
	...
	row := dbms.Db.QueryRowx("SELECT * FROM recommendation_resolution WHERE id = $1", query.ResolutionId)
	...
	row = dbms.Db.QueryRowx("SELECT * FROM recommendation_resolution WHERE id = $1", resolution.Id)
...
var recommendationJobProviderMap = map[string]string{
	...
	"spot_scan":               "nb",
	"popeye_scan":             "agent",
	"volume_analyzer":         "agent",
	"krr_scan":                "agent",
	"unused_pv":               "agent",
	"k8s_version_upgrade":     "agent",
	"certificate_scanner":     "agent",
	"kube_bench_scan":         "agent",
	"helm_chart_upgrade":      "agent",
	"trivy_cis_scan":          "agent",
}
...
func CreateRecommendationJob(ctx *security.RequestContext, query RecommendationJobCreateRequest) (RecommendationJobCreateResponse, error) {
	...
	case "agent":
		...
		// check agent_task tables to see if job is already submitted and not done, then dont allow to submit again
		rows, err := dbms.Db.Queryx("select count(*) as cnt from agent_task where cloud_account_id = $1 and tenant = $2 and action = $3 and status not in ('FAILED', 'COMPLETED', 'TIMEOUT')", query.AccountId, a.Tenant, query.JobName)
		if err != nil {
		...
		for rows.Next() {
			...
			if cnt > 0 {
				return RecommendationJobCreateResponse{
					Data: []any{
						map[string]any{
							"message": fmt.Sprintf("job already submitted - %s", query.JobName),
						},
					},
				}, nil
			}
		...
		payload := map[string]any{
			...
			"origin":        "callback",
			"timestamp":     time.Now(),
			"action_name":   query.JobName,
			"action_params": map[string]any{
				"a": "b",
			},
		}
		if query.JobName == "popeye_scan" {
		...
		task := map[string]any{
			"cloud_account_id": query.AccountId,
			"tenant":           a.Tenant,
			"action":           query.JobName,
			"payload":          string(payloadStr),
			"status":           "TODO",
			"source":           "recommendation",
		}
		_, err = dbms.Db.NamedExec(`INSERT INTO agent_task (cloud_account_id, tenant, action, payload, status, source) values (:cloud_account_id, :tenant, :action, :payload, :status, :source)`, []map[string]any{task})
		if err != nil {
			ctx.GetLogger().Error("error inserting image scanner tasks", "error", err)
			return RecommendationJobCreateResponse{}, err
		}
