...
func GenerateRecommendation(ctx *security.RequestContext, request GenerateRecommendationRequest) (GenerateRecommendationResponse, error) {
	...
	for _, acc := range accountList {
		...
		// processImageScanner inserts agent_task rows for the legacy image_scanner
		// action. Skip it for SERVER_ORCHESTRATED_SCANNERS tenants — the new
		// orchestrator path will handle per-image scans in Phase 2b. For now this
		// is a known temporary gap on flagged tenants.
		if !scan_orchestrator.IsEnabledForTenant(accountCtx, acc.TenantId) {
