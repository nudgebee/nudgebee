...
func IsFeatureEnabledByDefaultForAccount(ctx *security.RequestContext, tenantId string, accountId string, feature string) bool {
	...
	if err != nil && ctx != nil {
		ctx.GetLogger().Info("tenant: unable to fetch cached value, will be updating cache", "feature", feature, "tenantId", tenantId, "accountId", accountId)
	}
	...
	// No account row — fall back to tenant scope.
	return IsFeatureEnabledByDefault(ctx, tenantId, feature)
}
