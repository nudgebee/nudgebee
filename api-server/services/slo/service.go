...
func ExecuteSLO(accountId string) error {
	...
	rows, err := dbms.Db.Queryx(`SELECT * FROM public.slo_config where cloud_account_id=$1 and enabled = true`, accountId)
...
func getPreviousSLOReport(configId string, workload string, namespace string) (DBSLOReport, error) {
	...
	rows, err := dbms.Db.Queryx(`SELECT * FROM slo_report where config_id=$1 and workload_name=$2 and workload_namespace=$3 and status='FIRING' order by timestamp desc limit 1`, configId, workload, namespace)
...
func GenerateSLOEvent(dbms *database.DatabaseManager, sloConfig DBSLOConfig, accountName string) error {
	slo := DBSLOReport{}
	rows, err := dbms.Db.Queryx(`SELECT * FROM slo_report where config_id=$1 and workload_name=$2 and workload_namespace=$3 and status='FIRING' order by timestamp desc limit 1`, sloConfig.Id, sloConfig.WorkloadName, sloConfig.Namespace)
