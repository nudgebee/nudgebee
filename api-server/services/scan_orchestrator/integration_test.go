...
//
//	HASURA_ACTIONS_DB_URL=postgres://postgres:<pass>@localhost:5432/nudgebee?sslmode=disable
//
// (Use the cloud-sql-proxy tunnel that's already running on the dev box.)
func TestRelay_PersistEndToEnd(t *testing.T) {
	accountID, sa := setupRelay(t)
	dbURL := os.Getenv("HASURA_ACTIONS_DB_URL")
	if dbURL == "" {
		t.Skip("HASURA_ACTIONS_DB_URL not set — skipping persist verification (need cloud-sql-proxy tunnel)")
	}
	tenantID := os.Getenv("RELAY_TENANT_ID")
	if tenantID == "" {
	...
	// Snapshot existing rows for popeye_misconfigurations on this account so we
	// can show the delta after the run.
	dbms, err := database.GetDatabaseManager(database.Metastore)
	if err != nil {
		t.Fatalf("db: %v", err)
	}
	var beforeOpen, beforeArchive int
	_ = dbms.Db.Get(&beforeOpen, `SELECT COUNT(*) FROM recommendation WHERE cloud_account_id = $1 AND rule_name = $2 AND status = 'Open'`, accountID, PopeyeRuleName)
	_ = dbms.Db.Get(&beforeArchive, `SELECT COUNT(*) FROM recommendation WHERE cloud_account_id = $1 AND rule_name = $2 AND status = 'Archive'`, accountID, PopeyeRuleName)
	...
	if err := dbms.Db.Get(&afterOpen, `SELECT COUNT(*) FROM recommendation WHERE cloud_account_id = $1 AND rule_name = $2 AND status = 'Open'`, accountID, PopeyeRuleName); err != nil {
		t.Fatalf("post-run open count: %v", err)
	}
	if err := dbms.Db.Get(&afterArchive, `SELECT COUNT(*) FROM recommendation WHERE cloud_account_id = $1 AND rule_name = $2 AND status = 'Archive'`, accountID, PopeyeRuleName); err != nil {
	...
	var s sampleRow
	if err := dbms.Db.Get(&s, `SELECT category, severity, account_object_id, recommendation::text, recommendation_action FROM recommendation WHERE cloud_account_id = $1 AND rule_name = $2 AND status = 'Open' ORDER BY updated_at DESC LIMIT 1`, accountID, PopeyeRuleName); err != nil {
...
func TestRelay_PersistAllScanners(t *testing.T) {
	...
	dbURL := os.Getenv("HASURA_ACTIONS_DB_URL")
	if dbURL == "" {
		t.Skip("HASURA_ACTIONS_DB_URL not set")
	}
	tenantID := os.Getenv("RELAY_TENANT_ID")
	if tenantID == "" {
		t.Skip("RELAY_TENANT_ID not set")
