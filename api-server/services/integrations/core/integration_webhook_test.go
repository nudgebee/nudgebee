...
func TestExtractWebhookToken(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			url:      "/api/webhooks/grafana?token=aa4b9b31f0f2c516fd455105fe1fb722b26037305fa07e4fdbcf098a5849948a8c4fdcfa",
			expected: "aa4b9b31f0f2c516fd455105fe1fb722b26037305fa07e4fdbcf098a5849948a8c4fdcfa",
		},
		{
...
func TestParseAccountMapping(t *testing.T) {
	cases := []struct {
		...
		{
			name:     "flat canonical shape",
			raw:      `{"labelName":"env","dev":"a2a30b02-0f67-42e5-a2ab-c658230fd798","prod":"ff87fbfd-5729-4474-b9d6-96beb693e3fd"}`,
			expected: map[string]string{"labelName": "env", "dev": "a2a30b02-0f67-42e5-a2ab-c658230fd798", "prod": "ff87fbfd-5729-4474-b9d6-96beb693e3fd"},
		},
		{
		...
		{
			name:     "mixed flat + nested entries normalize",
			raw:      `{"labelName":"env","dev":"acc-1","prod":{"label":"k8s-prod","value":"acc-2"}}`,
			expected: map[string]string{"labelName": "env", "dev": "acc-1", "prod": "acc-2"},
		},
		{
			name:     "nested entry without value field is dropped",
			raw:      `{"labelName":"env","dev":{"label":"k8s-dev"}}`,
			expected: map[string]string{"labelName": "env"},
		},
		{
