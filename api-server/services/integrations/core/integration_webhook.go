...
// ParseAccountMapping extracts the account_mapping JSON from settings and returns
// the parsed map. Returns nil if the setting is missing or invalid.
//
// Tolerates two on-disk formats:
//
//  1. Flat (canonical):  {"labelName":"env","dev":"<acc-id>","prod":"<acc-id>"}
//  2. Nested (legacy):   {"labelName":"env","dev":{"label":"k8s-dev","value":"<acc-id>"}, ...}
//
// The nested shape was written by an older UI build that round-tripped a
// FilterDropdownButton option object straight into the payload. Normalize on
// read so callers always see the flat shape.
func ParseAccountMapping(settings []IntegrationConfigValue, logger *slog.Logger) map[string]string {
	raw, found := GetSettingValue(settings, "account_mapping")
	if !found || raw == "" {
		return nil
	}
	var rawMapping map[string]any
	if err := common.UnmarshalJson([]byte(raw), &rawMapping); err != nil {
		if logger != nil {
	...
	for k, v := range rawMapping {
		...
		case string:
			mapping[k] = val
		case map[string]any:
			// legacy nested form: pick the inner "value" string.
...
// It looks up the label specified by "labelName" (default "env") in the provided labels map
// and returns the mapped account ID. Returns the original accountId if no mapping matches.
func ApplyAccountMapping(accountId string, labels map[string]string, mapping map[string]string) string {
	if len(mapping) == 0 || len(labels) == 0 {
		return accountId
	}
	...
	if labelValue, ok := labels[labelName]; ok {
		if mappedAccountId, ok := mapping[labelValue]; ok && labelValue != "labelName" {
