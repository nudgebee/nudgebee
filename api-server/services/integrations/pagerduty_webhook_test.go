...
func TestResolveSubjectFromLabels(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			initialSubject:    "",
			labels:            map[string]string{"destination_workload_name": "cloud-collector-server", "destination_workload_namespace": "nudgebee-on-prem-test"},
			title:             "[FIRING:1] HighAPIFailureRate nudgebee (pd cloud-collector-server nudgebee-on-prem-test)",
			expectedSubject:   "cloud-collector-server",
			expectedNamespace: "nudgebee-on-prem-test",
		},
		{
...
func TestLLMLabelExtraction_Grafana(t *testing.T) {
	...
	parsedPayload := &core.EventIncomingWebhook{
		EventTitle:       "[FIRING:1] HighAPIFailureRate nudgebee (pd cloud-collector-server nudgebee-on-prem-test)",
		...
		Investigation: core.EventIncomingWebhookInvestigation{
			Labels: map[string]string{
				...
				"silenceURL":        "http://localhost:3000/alerting/silence/new?alertmanager=grafana&matcher=destination_workload_name%3Dcloud-collector-server&matcher=destination_workload_namespace%3Dnudgebee-on-prem-test",
...
func TestTools_GetCreatePagerDutyToolConfigs(t *testing.T) {
	...
	config, err := core.CreateIntegrationConfig(sc, "", IntegrationPagerdutyWebhook, toolConfigName, []core.IntegrationConfigValue{
		{
			Name:  "token",
			Value: "HctU3zmn3vdwZFUWjiCQp7wgcA2S5jeRGmo9",
		},
	},
		map[string]any{
			"env": "dev",
	...
	err = core.ProcessEventWebook(sc, "http://dev.nudgebee.pollux.in/api/webhooks/pagerduty?token=HctU3zmn3vdwZFUWjiCQp7wgcA2S5jeRGmo9", map[string]string{}, pagerDutyPayload)
	assert.Nil(t, err)
