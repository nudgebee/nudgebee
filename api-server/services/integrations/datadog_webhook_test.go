...
// Synthetics alert from xyz — has "null" incident fields and no monitor URL in body.
...
const datadogSyntheticsPayload = `{
	...
	"title": "[P3] [Triggered] [Synthetics] XYZ - A monitor that always fails",
	"date": "1773489470000",
	"org": { "id": "9999999", "name": "xyz-org" },
...
// Query alert monitor from xyz — has monitor URL in body but no API key available.
...
const datadogQueryAlertPayload = `{
	...
	"event_type": "query_alert_monitor",
	"title": "[Triggered] XYZ Dev - Web ASG At Max Capacity",
	"date": "1773528177000",
	"org": { "id": "9999999", "name": "xyz-org" },
	"body": "%%%\nWeb Auto Scaling Group has reached maximum capacity.\n\n[![Metric Graph](https://p.datadoghq.com/snapshot/view/dd-snapshots-prod/org_9999999/2026-03-14/abc.png)](https://app.datadoghq.com/monitors/266154777?from_ts=1773527277000&to_ts=1773528477000)\n\n**aws.autoscaling.group_in_service_instances** over **autoscalinggroupname:myapp-dev-web-asg** was **>= 3.0** on average during the **last 5m**.\n\n[[Monitor Status](https://app.datadoghq.com/monitors/266154777?from_ts=1773527277000&to_ts=1773528477000)] · [[Edit Monitor](https://app.datadoghq.com/monitors/266154777/edit)]\n%%%",
	"alert": { "alert_id": "266154777", "alert_metric": "aws.autoscaling.group_in_service_instances", "alert_query": "avg(last_5m):avg:aws.autoscaling.group_in_service_instances{autoscalinggroupname:myapp-dev-web-asg} >= 3", "alert_status": "", "alert_scope": "", "alert_transition": "Triggered", "alert_type": "error", "alert_metric_namespace": "aws" },
...
// Re-Triggered synthetics alert from xyz — the most common case (145/170 webhooks).
...
const datadogReTriggeredPayload = `{
	...
	"event_type": "synthetics_alert",
	"title": "[P4] [Re-Triggered] [Synthetics] API Test on myapp/api",
	"date": "1773500000000",
	"org": { "id": "9999999", "name": "xyz-org" },
	"body": "%%%\nAn alert has been received for myapp/api\n\n@webhook-Nudgebee\n%%%",
...
// Warn alert from xyz — status "warning" was previously dropped by routeWebhookEvent.
const datadogWarnPayload = `{
	"id": "8546376597550758824",
	"last_updated": "1773666546000",
	"event_type": "query_alert_monitor",
	"title": "[Warn] XYZ Dev - RDS High Write Latency",
	"date": "1773666546000",
	"org": { "id": "9999999", "name": "xyz-org" },
...
func TestDatadog_SyntheticsWithNullIncident(t *testing.T) {
	...
	assert.Equal(t, "[Synthetics] XYZ - A monitor that always fails", cleanTitle)
...
func TestDatadog_QueryAlertNoApiKey(t *testing.T) {
	...
	assert.Len(t, titleMatches, 4)
	cleanTitle := titleMatches[3]
	assert.Equal(t, "XYZ Dev - Web ASG At Max Capacity", cleanTitle)
...
func TestDatadog_WarnStatus(t *testing.T) {
	...
	cleanTitle := titleMatches[3]
	assert.Equal(t, "XYZ Dev - RDS High Write Latency", cleanTitle)
...
// correctly — this is the most common case in xyz data (145/170 webhooks).
func TestDatadog_ReTriggeredSynthetics(t *testing.T) {
	...
	assert.Equal(t, "[Synthetics] API Test on myapp/api", titleMatches[3], "cleanTitle should strip prefix brackets")
